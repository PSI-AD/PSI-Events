...
const TIER_CFG = {
    ...
    Average: { label: 'Average', color: 'text-slate-400', bg: 'bg-slate-400/10', border: 'border-slate-600/30', dot: 'bg-slate-500', icon: <Users size={11} /> },
};
...
const TIER_SPONSOR = {
    ...
    Silver: { bg: 'from-slate-400/20 to-slate-500/5', border: 'border-slate-400/40', text: 'text-slate-300' },
...
function KPICard({ icon, label, value, sub, color, delay = 0, pulse = false }: KPICardProps) {
    return (
        <motion.div
            ...
            className="relative bg-white/5 border border-white/10 rounded-3xl p-6 overflow-hidden group hover:bg-white/[0.07] transition-colors"
        >
            ...
            <p className={`text-3xl md:text-4xl font-extrabold tracking-tight text-white mb-1`}>{value}</p>
            ...
            <div className="space-y-3">
                {tiers.map((t, i) => (
                    ...
                    >
                        ...
                        <span className="text-white font-extrabold text-sm font-mono">{t.count}</span>
                        <span className="text-white/30 text-xs w-8 text-right">{t.pct}%</span>
                    </motion.div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
            <div className="flex items-center gap-2 mb-5">
                ...
                <span className="ml-auto text-white/25 text-xs font-mono">{leads.length} total</span>
            </div>
                    ...
                    return (
                        <div key={project}>
                            <div className="flex items-center justify-between mb-1.5">
                                <span className="text-sm font-bold text-white/80 truncate pr-3">{project}</span>
                                <span className="text-white font-extrabold text-sm font-mono flex-shrink-0">{count}</span>
                            </div>
...
function LiveFeed({ entries }: { entries: FeedEntry[] }) {
    ...
    return (
        <div className="bg-white/5 border border-white/10 rounded-3xl flex flex-col overflow-hidden" style={{ maxHeight: '460px' }}>
            {/* Feed header */}
            <div className="flex items-center gap-2 px-6 py-4 border-b border-white/8 flex-shrink-0">
                ...
                <span className="ml-auto text-white/25 text-xs font-mono">{entries.length} events</span>
            </div>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm text-white/80 font-medium leading-snug">
                                        ...
                                        <span className="text-white font-bold">{entry.project}</span>
                                        {' '}by Agent{' '}
                                        <span className="font-mono text-white/60">{entry.agentInitials}</span>
                                    </p>
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <span className={`text-[10px] font-bold ${tc.color}`}>{tc.label}</span>
                                        <span className="text-white/20 text-[10px]">·</span>
                                        <span className="text-white/30 text-[10px]">{SOURCE_LABEL[entry.source] ?? entry.source}</span>
                                    </div>
                                </div>

                                {/* Time */}
                                <span className="flex-shrink-0 text-white/25 text-[10px] font-mono">{entry.time}</span>
                            </motion.div>
                        );
                    })}
                </AnimatePresence>
        ...
        <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center px-6">
            ...
            <div className="relative w-full max-w-md">
                ...
                >
                    ...
                    <div>
                        <p className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">Property Shop Investment LLC</p>
                        <p className="text-base font-extrabold text-white leading-tight">Sponsor ROI Portal</p>
                    </div>
                </motion.div>
                ...
                >
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl font-extrabold text-white mb-2">Secure Access</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="sponsor-token-input" className="block text-[10px] font-bold uppercase tracking-widest text-white/40 mb-2">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 pr-12 text-sm text-white placeholder-white/20 font-mono focus:outline-none focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all"
                                    autoComplete="off"
                                    spellCheck={false}
                                />
                                <button
                        ...
                        <motion.button
                            ...
                            disabled={loading}
                            whileTap={{ scale: 0.98 }}
                            className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:opacity-50 text-white font-extrabold text-base shadow-xl shadow-amber-500/20 transition-all"
                        >
                            {loading ? (
                    ...
                    <div className="mt-5 pt-5 border-t border-white/8 text-center">
                        <button
                            id="demo-token-btn"
                            onClick={loadDemo}
                            className="text-xs text-white/30 hover:text-amber-400/70 transition-colors font-semibold underline underline-offset-2"
                        >
        ...
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8 text-center text-white">
            <div>
                ...
                <p className="text-white/50 max-w-sm mx-auto">
                    {tokenName && <span className="font-bold text-white">{tokenName}'s </span>}
...
function DashboardView({ token, useDemoData = true }: DashboardViewProps) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white font-sans">
            ...
            {/* ── HEADER ─────────────────────────────────────────────────────── */}
            <header className="relative z-10 border-b border-white/8 bg-slate-950/80 backdrop-blur-sm">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden sm:block">
                            <p className="text-[9px] font-bold text-white/25 uppercase tracking-[0.2em]">Property Shop Investment</p>
                            <p className="text-sm font-extrabold text-white leading-tight">Sponsor ROI Portal</p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div>
                            <p className="text-sm font-extrabold text-white leading-tight">{token.sponsorName}</p>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {lastUpdated && (
                            <div className="hidden lg:flex items-center gap-1.5 text-white/25 text-[10px] font-mono">
                                <Clock size={10} />
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold uppercase tracking-widest text-white/50 mb-4"
                            >
                            ...
                            >
                                ...
                                <br />
                                <span className="text-white/70 text-2xl md:text-3xl">Dashboard</span>
                            </motion.h1>
                            ...
                            <motion.p
                                ...
                                className="text-white/40 text-sm mt-3 max-w-xl"
                            >
                        ...
                        <motion.div
                            ...
                            className="flex items-start gap-3 bg-white/5 border border-white/10 rounded-2xl px-5 py-4 max-w-xs"
                        >
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-6 py-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                        ...
                        >
                            ...
                            <p className="text-[10px] font-bold uppercase tracking-widest text-white/35 mb-1">{item.title}</p>
                            <p className="text-xl font-extrabold text-white mb-1">{item.value}</p>
                ...
                {/* ── FOOTER DISCLAIMER ─────────────────────────────────────── */}
                <div className="border-t border-white/8 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-white/20">

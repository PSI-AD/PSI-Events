        ...
        <AnimatePresence>
            {open && (
                <>
                    ...
                    >
                        <div className="bg-slate-900 border border-white/12 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden">
                            {/* Search bar */}
                            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/8">
                                ...
                                />
                                <kbd className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 bg-white/8 border border-white/10 rounded text-[10px] text-white/30 font-mono">
                                    ESC
                                </kbd>
                            </div>
                            ...
                            <div className="max-h-[56vh] overflow-y-auto p-2">
                                {groups.length === 0 ? (
                                    ...
                                    <div key={group} className="mb-2">
                                        <p className="px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.2em] text-white/20">{group}</p>
                                        {results.filter(r => r.group === group).map(item => (
                                            ...
                                            >
                                                <span className="w-7 h-7 rounded-lg bg-white/6 flex items-center justify-center text-white/50 flex-shrink-0 group-hover:text-white transition-colors">
                                                    {item.icon}
                                                </span>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-white text-sm font-semibold truncate">{item.label}</p>
                                                    {item.description && <p className="text-white/30 text-[11px] truncate">{item.description}</p>}
                                                </div>
                                                <ChevronRight size={12} className="text-white/15 flex-shrink-0 group-hover:text-white/40 transition-colors" />
                                            </button>
                                        ))}
                                    </div>
                                ))}
            ...
            {open && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-14 right-3 sm:right-4 w-[calc(100vw-1.5rem)] sm:w-96 z-[71] max-h-[80vh] flex flex-col bg-slate-900 border border-white/10 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden"
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/8">
                            ...
                            <div className="flex items-center gap-2">
                                {unread > 0 && (
                                    <button onClick={markAllRead} className="text-white/30 hover:text-white text-[10px] font-bold transition-colors">
                                        Mark all read
                                    </button>
                                )}
                                <button onClick={onClose} className="w-7 h-7 rounded-lg bg-white/6 border border-white/10 flex items-center justify-center text-white/40 hover:text-white transition-all">
                                    <X size={12} />
                                </button>
                            </div>
                        </div>
                            ...
                            {notifs.map(n => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`flex items-start gap-3 px-4 py-3 border-b border-white/5 cursor-pointer transition-all hover:bg-white/4 ${!n.read ? 'bg-white/3' : ''}`}
                                    >
                                        <div className={`w-7 h-7 rounded-xl ${cfg.bg} ${cfg.color} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                                            {cfg.icon}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-sm font-bold leading-tight ${n.read ? 'text-white/60' : 'text-white'}`}>{n.title}</p>
                                            <p className="text-white/35 text-[11px] leading-relaxed mt-0.5 line-clamp-2">{n.body}</p>
                                            <div className="flex items-center gap-2 mt-1">
                                                <Clock size={9} className="text-white/20" />
                                                <span className="text-white/20 text-[9px]">{n.relativeTime}</span>
                                                {n.link && <span className="text-white/20 text-[9px] flex items-center gap-0.5"><ArrowRight size={8} /> view</span>}
                                            </div>
                                        </div>
                        ...
                        <div className="border-t border-white/8 px-4 py-3 text-center">
                            <p className="text-white/20 text-[10px]">Notifications sync live · powered by Firestore</p>
                        </div>
                    </motion.div>
                </>
            )}
        ...
        <AnimatePresence>
            {open && (
                <motion.div
                    ...
                    transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                    className="fixed bottom-[5.5rem] md:bottom-6 right-3 sm:right-4 w-[calc(100vw-1.5rem)] sm:w-96 z-[72] flex flex-col bg-slate-900 border border-white/10 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden"
                ...
                >
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-white/8 bg-gradient-to-r from-indigo-600/20 to-violet-600/20">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-white font-extrabold text-sm">PSI AI Assistant</p>
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] rounded-2xl px-3 py-2.5 ${msg.role === 'user'
                                        ...
                                        : 'bg-white/6 border border-white/8 text-white/85 text-sm'
                                    }`}>
                                    {/* Render **bold** markdown */}
                                    <p className="leading-relaxed whitespace-pre-wrap text-[13px]"
                                        dangerouslySetInnerHTML={{ __html: msg.content.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`(.+?)`/g, '<code class="bg-white/10 px-1 rounded text-xs font-mono">$1</code>') }}
                                    />
                        ...
                        <div className="px-4 pb-2 flex gap-1.5 overflow-x-auto scrollbar-none">
                            {QUICK.map(q => (
                                <button key={q} onClick={() => { setInput(q); setTimeout(handleSend, 50); }}
                                    className="flex-shrink-0 px-2.5 py-1.5 bg-white/5 border border-white/10 rounded-xl text-white/50 text-[10px] font-semibold hover:bg-white/10 hover:text-white transition-all whitespace-nowrap">
                                    {q}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="border-t border-white/8 p-3 flex items-center gap-2">
                        <input
                            ...
                            className="flex-1 bg-white/6 border border-white/10 rounded-xl px-3 py-2 text-white text-xs placeholder:text-white/25 outline-none focus:border-indigo-500/50 transition-colors"
                        />
                        <button
                            id="ai-chat-send"
                            onClick={handleSend}
                            disabled={!input.trim() || loading}
                            className="w-8 h-8 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-30 text-white flex items-center justify-center transition-all"
                        >
                            <Send size={13} />
                        </button>
                    </div>
    ...
    return (
        <div className={`flex items-center gap-1.5 ${className}`}>
            ...
            <button id="cmd-palette-btn" onClick={() => setCmdOpen(true)}
                ...
                className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white/6 hover:bg-white/10 border border-white/10 rounded-xl text-slate-400 hover:text-white text-xs font-medium transition-all">
                <Search size={13} />
                <span className="hidden md:inline">Search…</span>
                <kbd className="hidden md:flex items-center gap-0.5 text-[9px] font-mono bg-white/10 px-1 rounded">
                    <Command size={8} />K
                </kbd>
            </button>
            <button id="cmd-palette-btn-mobile" onClick={() => setCmdOpen(true)}
                aria-label="Search"
                className="sm:hidden p-2 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center">
                <Search size={18} />
            </button>
            ...
            <button id="notifications-btn" onClick={() => setNotifOpen(!notifOpen)}
                ...
                className="relative p-2 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-white transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center">
                <Bell size={18} className={notifOpen ? 'text-white' : ''} />
                {unreadCount > 0 && (
            ...
            <button id="ai-chat-btn" onClick={() => setChatOpen(!chatOpen)}
                ...
                className={`p-2 rounded-xl transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center ${chatOpen ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>

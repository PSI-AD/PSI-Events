            ...
            >
                ...
                <div className="flex items-center justify-between px-4 py-3 flex-shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <p className="text-psi-primary font-semibold text-sm truncate max-w-md">{session.title}</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={() => onBookmark(session.id)}
                            className={cn('p-2 rounded-lg transition-colors', isBookmarked ? 'text-amber-400' : 'text-psi-secondary hover:text-psi-primary')}>
                ...
                <div className="flex-1 flex items-center justify-center px-4 overflow-hidden">
                    <div className={cn('w-full aspect-video max-h-[55vh] bg-gradient-to-br rounded-2xl flex items-center justify-center relative overflow-hidden', session.thumbnailColor)}>
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-psi-secondary text-sm font-medium">Session Recording</p>
                            <p className="text-psi-muted text-xs mt-1">{session.title}</p>
                        </div>
                        ...
                        {!playing && (
                            ...
                            >
                                <div className="w-20 h-20 rounded-full bg-psi-border backdrop-blur border border-white/30 flex items-center justify-center hover:scale-105 transition-transform">
                                    <Play size={32} className="text-psi-primary ml-1" />
                                </div>
                            </button>
                        )}
                        ...
                        {speakers.length > 0 && (
                            <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/40 backdrop-blur px-3 py-1.5 rounded-lg">
                                ...
                                <div>
                                    <p className="text-psi-primary text-xs font-semibold">{speakers[0].name}</p>
                                    <p className="text-psi-secondary text-[10px]">{speakers[0].company}</p>
                                </div>
                            </div>
                        )}
                ...
                <div className="px-4 pb-4 flex-shrink-0">
                    ...
                    <div className="mb-3 flex items-center gap-3">
                        <span className="text-psi-secondary text-xs font-mono w-10 text-right">{formatTime(currentSec)}</span>
                        <div
                            className="flex-1 h-2 bg-psi-border rounded-full cursor-pointer relative group"
                            onClick={handleSeek}
                        >
                        ...
                        <span className="text-psi-secondary text-xs font-mono w-10">{session.duration}</span>
                    </div>

                    {/* Buttons row */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <button onClick={() => skip(-15)} className="p-2 rounded-lg text-psi-secondary hover:text-psi-primary hover:bg-psi-subtle transition-colors">
                                <SkipBack size={16} />
                            </button>
                            <button
                            ...
                            <button onClick={() => setMuted(m => !m)} className="p-2 rounded-lg text-psi-secondary hover:text-psi-primary hover:bg-psi-subtle transition-colors">
                                {muted ? <VolumeX size={16} /> : <Volume2 size={16} />}
                            </button>
                        </div>
                        <div className="flex items-center gap-1 text-psi-secondary text-xs">
                ...
                <div className="border-t border-psi flex-shrink-0 max-h-[28vh] overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-white/10">
                        {/* Takeaways */}
                        <div className="px-5 py-4">
                            <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-2">Key Takeaways</p>
                            <ul className="space-y-1.5">
                                {session.keyTakeaways.map((t, i) => (
                                    <li key={i} className="flex items-start gap-2 text-xs text-psi-secondary">
                        ...
                        {/* Documents */}
                        <div className="px-5 py-4">
                            <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-2">Session Documents</p>
                            <div className="space-y-2">
                                {session.documents.map(d => (
                                    <button key={d.id}
                                        onClick={() => toast.success(`Downloading: ${d.name}`)}
                                        className="w-full flex items-center gap-2 hover:bg-psi-subtle rounded-lg px-2 py-1.5 transition-colors text-left">
                                        <span className="text-base">{d.emoji}</span>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-psi-primary text-xs truncate">{d.name}</p>
                                            <p className="text-psi-secondary text-[10px]">{d.type} · {d.sizeLabel}</p>
                                        </div>
                                        <Download size={11} className="text-psi-secondary flex-shrink-0" />
                                    </button>
                                ))}
                            </div>
                        </div>
...
function SessionCard({
    ...
    return (
        <motion.div
            ...
            className="group bg-psi-surface border border-psi hover:border-psi-strong rounded-2xl overflow-hidden transition-colors"
        >
            {/* Thumbnail */}
            <div
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="w-12 h-12 rounded-full bg-psi-border backdrop-blur border border-white/30 flex items-center justify-center">
                        <Play size={20} className="text-psi-primary ml-0.5" />
                    </div>
                </div>
            ...
            {/* Content */}
            <div className="p-4">
                <h3 className="text-psi-primary font-bold text-sm leading-snug mb-2 line-clamp-2 group-hover:text-emerald-300 transition-colors cursor-pointer"
                    onClick={onPlay}>
                    {session.title}
                </h3>

                {!compact && (
                    <p className="text-psi-secondary text-xs leading-relaxed mb-3 line-clamp-2">{session.description}</p>
                )}
                    ...
                    <div className="flex items-center gap-1.5 mb-3">
                        {speakers.map(sp => (
                            <div key={sp.id} className="flex items-center gap-1.5">
                                ...
                                <span className="text-psi-secondary text-xs">{sp.name}</span>
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        <button onClick={() => onBookmark(session.id)}
                            className={cn('p-1.5 rounded-lg transition-colors', isBookmarked ? 'text-amber-400' : 'text-psi-muted hover:text-psi-primary')}>
...
function SpeakerCard({
    ...
    return (
        <motion.div
            ...
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-psi-surface border border-psi hover:border-psi-strong rounded-2xl p-5 transition-colors group"
        >
            <div className="flex items-start gap-4 mb-4">
                <div className="w-14 h-14 rounded-2xl bg-psi-subtle border border-psi-strong flex items-center justify-center text-3xl flex-shrink-0">{speaker.avatar}</div>
                <div className="flex-1 min-w-0">
                    <h3 className="text-psi-primary font-bold text-sm">{speaker.name}</h3>
                    <p className="text-psi-secondary text-xs mt-0.5">{speaker.title}</p>
                    <p className="text-psi-secondary text-xs">{speaker.company} · {speaker.country}</p>
                </div>
                <button onClick={() => onBookmark(speaker.id)}
                    className={cn('p-1.5 rounded-lg transition-colors flex-shrink-0', isBookmarked ? 'text-amber-400' : 'text-psi-muted hover:text-psi-primary')}>
                    {isBookmarked ? <BookmarkCheck size={13} /> : <Bookmark size={13} />}
                </button>
            </div>

            <p className="text-psi-secondary text-xs leading-relaxed mb-3 line-clamp-3">{speaker.bio}</p>

            <div className="flex flex-wrap gap-1 mb-4">
                {speaker.topics.map(t => (
                    <span key={t} className="text-[9px] bg-psi-subtle border border-psi-strong text-psi-secondary px-2 py-0.5 rounded-full">{t}</span>
                ))}
            </div>

            <div className="flex items-center justify-between">
                <span className="text-psi-muted text-xs">{sessions.length} session{sessions.length !== 1 ? 's' : ''}</span>
    ...
    return (
        ...
        >
            <motion.div
                ...
                onClick={e => e.stopPropagation()}
                className="w-full max-w-md bg-psi-page border-l border-psi overflow-y-auto flex flex-col"
            >
                <div className="p-6 border-b border-psi flex items-start justify-between gap-3">
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-2xl bg-psi-subtle border border-psi-strong flex items-center justify-center text-4xl">{speaker.avatar}</div>
                        <div>
                            <h2 className="text-psi-primary font-black text-lg">{speaker.name}</h2>
                            <p className="text-psi-secondary text-sm">{speaker.title}</p>
                            <p className="text-psi-secondary text-xs">{speaker.company}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-lg text-psi-secondary hover:text-psi-primary hover:bg-psi-subtle transition-colors flex-shrink-0">
                        <X size={16} />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-6 flex-1">
                    <div>
                        ...
                        <p className="text-psi-secondary text-sm leading-relaxed">{speaker.bio}</p>
                    </div>
                    <div>
                    ...
                    {sessions.length > 0 && (
                        <div>
                            <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Sessions ({sessions.length})</p>
                            <div className="space-y-2">
                                {sessions.map(s => (
                                    <button key={s.id} onClick={() => onPlaySession(s)}
                                        className="w-full text-left flex items-center gap-3 bg-psi-surface border border-psi rounded-xl p-3 hover:border-psi-strong transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-psi-primary text-xs font-semibold truncate">{s.title}</p>
                                            <p className="text-psi-secondary text-[10px]">{s.duration} · {s.views.toLocaleString()} views</p>
                                        </div>
...
function DocumentRow({ doc }: { doc: ReturnType<typeof getAllDocuments>[0] }) {
    return (
        <div className="flex items-center gap-3 bg-psi-surface border border-psi rounded-xl p-3 hover:border-psi-strong transition-colors">
            <span className="text-2xl">{doc.emoji}</span>
            <div className="flex-1 min-w-0">
                <p className="text-psi-primary text-sm font-semibold truncate">{doc.name}</p>
                <p className="text-psi-secondary text-xs">{doc.type} · {doc.sizeLabel} · {doc.downloads.toLocaleString()} downloads</p>
            </div>
            <button onClick={() => toast.success(`Downloading: ${doc.name}`)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-xs font-semibold hover:bg-emerald-600 hover:border-emerald-600 hover:text-white transition-all flex-shrink-0">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-psi-primary font-extrabold text-base">Post-Event Content Hub</h1>
                        </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                                ...
                                <button key={ev.id} onClick={() => { setActiveEvent(ev); setQuery(''); }}
                                    className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all',
                                        activeEvent.id === ev.id
                                            ...
                                            : 'bg-psi-subtle border-psi-strong text-psi-secondary hover:text-psi-primary')}>
                        ...
                        <div className="relative">
                            ...
                            <input value={query} onChange={e => setQuery(e.target.value)}
                                ...
                                className="bg-psi-subtle border border-psi-strong rounded-xl pl-8 pr-3 py-2 text-xs text-psi-primary placeholder-slate-500 focus:outline-none focus:border-violet-500 w-44 transition-colors" />
                        </div>

                        {/* Grid/List toggle */}
                        <div className="flex bg-psi-subtle border border-psi-strong rounded-xl p-0.5 gap-0.5">
                            <button onClick={() => setGridMode(true)}
                                className={cn('p-1.5 rounded-lg transition-colors', gridMode ? 'bg-psi-border text-psi-primary' : 'text-psi-secondary')}>
                                <LayoutGrid size={13} />
                            </button>
                ...
                {([
                    ...
                    <button key={t.id} onClick={() => setViewTab(t.id)}
                        className={cn('flex items-center gap-1.5 px-4 py-3 text-sm font-semibold border-b-2 transition-colors',
                            viewTab === t.id ? 'border-violet-500 text-violet-400' : 'border-transparent text-psi-secondary hover:text-psi-primary')}>
                        <t.icon size={13} /> {t.label}
                        <span className={cn('text-[9px] px-1.5 py-0.5 rounded-full font-black', viewTab === t.id ? 'bg-violet-500/20 text-violet-400' : 'bg-psi-border text-psi-secondary')}>
                            {t.count}
                        </span>
                    </button>
                ))}
            ...
            {activeEvent.status !== 'Published' ? (
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <h2 className="text-psi-primary font-black text-xl mb-2">{activeEvent.name}</h2>
                        <p className="text-psi-secondary text-sm max-w-sm">{activeEvent.description}</p>
                                ...
                                {bookmarks.size === 0 ? (
                                    ...
                                    <div className="space-y-8">
                                        {bookmarkedSessions.length > 0 && (
                                            <div>
                                                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Saved Sessions ({bookmarkedSessions.length})</p>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                                        ...
                                        {bookmarkedSpeakers.length > 0 && (
                                            <div>
                                                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Saved Speakers ({bookmarkedSpeakers.length})</p>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">

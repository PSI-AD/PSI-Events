            ...
            <div className="relative w-20 h-20">
                <svg className="w-20 h-20 -rotate-90" viewBox="0 0 88 88">
                    <circle cx="44" cy="44" r={radius} stroke="currentColor" strokeWidth="6"
                        className="text-psi-primary/10" fill="none" />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-psi-primary font-extrabold text-lg tabular-nums">{displayed}</span>
                </div>
            </div>
            <div className="flex items-center gap-1 text-psi-secondary text-[10px] font-bold uppercase tracking-widest">
                {icon}
                <span>{label}</span>
            </div>
        </motion.div>
        ...
        <div className="relative flex items-center justify-center">
            ...
            <motion.button
                ...
                className={[
                    ...
                    'text-psi-primary font-bold text-2xl transition-all shadow-2xl',
                    isRecording
    ...
    return (
        <div className="flex flex-col items-center gap-6 py-8">
            ...
            <div className="text-center">
                <p className="text-psi-primary font-extrabold text-lg mb-2">{label}</p>
                <AnimatePresence mode="wait">
                    <motion.p
                        ...
                        exit={{ opacity: 0, y: -4 }}
                        className="text-psi-secondary text-sm"
                    >
                        {dots[idx]}
                    </motion.p>
                </AnimatePresence>
...
export default function AIPitchSimulator({
        ...
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex flex-col items-center justify-center px-4 py-8">
            ...
            >
                ...
                <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    onClick={handleStart}
                    className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-psi-primary font-extrabold text-base shadow-xl shadow-violet-600/25 hover:shadow-violet-600/40 transition-all"
                >
                    <Mic size={20} />
                    Start Simulation
    ...
    if (phase === 'record') return (
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex flex-col items-center justify-center px-4 py-8 text-psi-primary">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    {Array.from({ length: MAX_ROUNDS }).map((_, i) => (
                        <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === round ? 'bg-violet-400 w-6' : i < round ? 'bg-violet-600' : 'bg-psi-border-subtle'}`} />
                    ))}
                    <span className="text-psi-muted text-xs ml-1 font-mono">
                        Attempt {round + 1}/{MAX_ROUNDS}
                    </span>
                </div>

                {/* Scenario card */}
                <div className="w-full bg-psi-subtle border border-psi rounded-3xl p-6 text-center">
                    ...
                    <h2 className="text-psi-primary text-xl font-extrabold leading-tight mb-3">
                        {scenario.title}
                    </h2>
                    <p className="text-psi-secondary text-sm leading-relaxed">
                        🎭 <span className="text-psi-primary/70 font-semibold">Playing as:</span> {scenario.clientProfile}
                    </p>
                    <p className="text-psi-secondary text-xs leading-relaxed mt-2">
                ...
                <div className="flex flex-col items-center gap-5">
                    ...
                    <p className="text-psi-muted text-sm text-center">
                ...
                <AnimatePresence>
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-psi-subtle border border-psi text-psi-primary/70 text-sm font-bold hover:bg-psi-subtle transition-all"
                                >
                                ...
                                <motion.button
                                    ...
                                    className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-psi-primary font-extrabold text-sm shadow-lg shadow-violet-600/25 hover:shadow-violet-600/40 transition-all"
                                >
                    ...
                    {recorder.isRecording && (
                        <motion.button
                            ...
                            whileTap={{ scale: 0.97 }}
                            onClick={handleStopAndAnalyse}
                            className="flex items-center justify-center gap-2 px-8 py-3.5 rounded-2xl bg-rose-600 hover:bg-rose-500 text-psi-primary font-extrabold text-sm shadow-lg shadow-rose-600/30 transition-all"
                        >
                ...
                <button
                    ...
                    className="text-psi-muted hover:text-psi-primary/50 text-xs transition-colors"
                >
                    ← Back to scenarios
                </button>
            </motion.div>
    ...
    if (phase === 'thinking') return (
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex items-center justify-center px-4">
    ...
    if (phase === 'objection') return (
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex flex-col items-center justify-center px-4 py-8 text-psi-primary">
            ...
            >
                {/* AI Client avatar */}
                <div className="text-center">
                    <div className="w-16 h-16 rounded-3xl bg-psi-subtle border-2 border-white/15 flex items-center justify-center mx-auto mb-3 text-3xl">
                        🧑‍💼
                    </div>
                    <p className="text-psi-muted text-[10px] font-bold uppercase tracking-widest">AI Buyer — {scenario.clientProfile}</p>
                </div>
                ...
                <motion.div
                    ...
                    className="bg-psi-subtle border border-psi rounded-3xl rounded-tl-lg p-6"
                >
                    ...
                    <p className="text-psi-primary text-lg font-bold leading-relaxed italic">
                        "{objectionText}"
                    </p>
                </motion.div>
                ...
                {scoreData?.transcript && (
                    <div className="bg-violet-500/5 border border-violet-500/15 rounded-2xl p-4">
                        ...
                        <p className="text-psi-secondary text-xs leading-relaxed italic">{scoreData.transcript}</p>
                    </div>
                )}
                ...
                <motion.button
                    ...
                    disabled={saving}
                    className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 text-psi-primary font-extrabold text-base shadow-xl shadow-amber-500/25 disabled:opacity-60 transition-all"
                >
                    {saving
    ...
    if (phase === 'result_pass' && scoreData) return (
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex flex-col items-center justify-center px-4 py-8 text-psi-primary overflow-hidden">
            ...
            >
                ...
                <div className="text-center">
                    ...
                    <h2 className="text-3xl font-extrabold text-psi-primary mb-1">Assessment Passed!</h2>
                ...
                <div className="bg-psi-subtle border border-psi rounded-3xl p-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-psi-muted text-center mb-5">Score Breakdown</p>
                ...
                <div className="bg-violet-500/10 border border-violet-500/20 rounded-2xl p-4">
                    ...
                    <p className="text-psi-primary/70 text-xs leading-relaxed">{scoreData.feedback}</p>
                </div>
                ...
                >
                    ...
                    <div>
                        ...
                        <p className="text-psi-secondary text-xs mt-0.5">`assessment_passed: true` written to your event record.</p>
                    </div>
                </motion.div>
                ...
                <button
                    id="sim-done-btn"
                    onClick={handleRestart}
                    className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-psi-subtle border border-psi text-psi-secondary text-sm font-bold hover:bg-psi-subtle transition-all"
                >
        ...
        <div className="min-h-[calc(100vh-4rem)] bg-psi-page flex flex-col items-center justify-center px-4 py-8 text-psi-primary">
            ...
            >
                <div className="text-center">
                    ...
                    <h2 className="text-2xl font-extrabold text-psi-primary mb-1">Not Quite There Yet</h2>
                    <p className="text-psi-muted text-sm">Best score: <span className="text-amber-400 font-black">{bestScore}%</span> · Threshold: {PASS_THRESHOLD}%</p>
                </div>

                {/* Score rings */}
                <div className="bg-psi-subtle border border-psi rounded-3xl p-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-psi-muted text-center mb-5">Latest Attempt</p>
                    <div className="grid grid-cols-3 gap-4">
                        <ScoreRing score={scoreData.confidence} label="Confidence" icon={<Mic size={9} />} delay={0} />
                        <ScoreRing score={scoreData.projectKnowledge} label="Knowledge" icon={<Brain size={9} />} delay={200} />
                        <ScoreRing score={scoreData.objectionHandling} label="Objections" icon={<Target size={9} />} delay={400} />
                ...
                <div className="bg-amber-500/8 border border-amber-500/15 rounded-2xl p-4">
                    ...
                    <p className="text-psi-secondary text-xs leading-relaxed">{scoreData.feedback}</p>
                </div>
                ...
                <div className="space-y-2">
                    {scenario.tips.slice(0, 2).map((tip, i) => (
                        <div key={i} className="flex items-start gap-2 px-3 py-2 bg-white/[0.03] rounded-xl border border-white/[0.06]">
                            ...
                            <p className="text-psi-secondary text-xs">{tip}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button
                        ...
                        onClick={handleRestart}
                        className="flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-psi-subtle border border-psi text-psi-secondary text-sm font-bold hover:bg-psi-subtle transition-all"
                    >
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        onClick={handleRestart}
                        className="flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-violet-500 text-psi-primary font-extrabold text-sm shadow-lg shadow-violet-600/20 transition-all"
                    >

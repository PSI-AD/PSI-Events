...
function ZoneBadge({ zone, count, total }: { zone: FloorZone; count: number; total: number }) {
    ...
    return (
        <motion.div
            ...
            <div className={clsx('flex flex-col items-center gap-0.5 px-2.5 py-1.5 rounded-xl border border-white/20 shadow-lg backdrop-blur-sm', zone.colour, 'bg-opacity-85')}>
                <span className="text-white font-black text-lg leading-none">{count}</span>
                <span className="text-white/80 text-[9px] font-bold leading-none">{pct}%</span>
                <span className="text-white/60 text-[8px] uppercase tracking-widest leading-none">{zone.name.split('—')[0].trim()}</span>
            </div>
        </motion.div>
    );
}
            ...
            {imageUrl && (
                <div className="psi-card rounded-2xl p-5 space-y-4">
                    ...
                    <div ref={mapRef} onClick={handleMapClick}
                        ...
                        <img src={imageUrl} alt="Floorplan" className="w-full h-auto block max-h-72 object-contain bg-slate-900" />
                        {zones.map(z => (
                            <div key={z.id} style={{ left: `${z.x * 100}%`, top: `${z.y * 100}%` }} className="absolute -translate-x-1/2 -translate-y-1/2 group">
                                ...
                                <div className="absolute left-1/2 -translate-x-1/2 bottom-6 hidden group-hover:block bg-slate-900 text-white text-[10px] font-bold px-2 py-1 rounded-lg whitespace-nowrap shadow-xl border border-slate-700">{z.name}</div>
                            </div>
                        ))}
                        {pendingPos && (
                            <motion.div style={{ left: `${pendingPos.x * 100}%`, top: `${pendingPos.y * 100}%` }}
                                ...
                                <div className="w-6 h-6 rounded-full bg-amber-500 border-2 border-white shadow-xl animate-bounce flex items-center justify-center"><Plus size={12} className="text-white" /></div>
                            </motion.div>
                        )}
                    </div>
                    {zones.length > 0 && (
                        <div className="space-y-2">
                            {zones.map((z, i) => (
                                <div key={z.id} className="flex items-center gap-3 p-2.5 bg-psi-subtle rounded-xl">
                                    <div className={clsx('w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0', z.colour)}><span className="text-white text-[10px] font-black">{i + 1}</span></div>
        ...
        <div className="space-y-4">
            <div className="psi-card rounded-2xl p-4">
                ...
                <div ref={mapRef} onClick={handleMapTap} onTouchStart={handleMapTap}
                    ...
                    <img src={imageUrl} alt="Floorplan" className="w-full h-auto block max-h-60 object-contain bg-slate-900" />
                    {ripples.map(r => (
                    ...
                    <AnimatePresence>{tapPos && (
                        <motion.div style={{ left: `${tapPos.x * 100}%`, top: `${tapPos.y * 100}%` }}
                            ...
                            <div className="relative">
                                <div className="w-7 h-7 rounded-full bg-amber-500 border-2 border-white shadow-xl flex items-center justify-center"><MapPin size={14} className="text-white" /></div>
                    ...
                    {zones.map(z => (
                        <div key={z.id} style={{ left: `${z.x * 100}%`, top: `${z.y * 100}%` }} className="absolute -translate-x-1/2 -translate-y-1/2 pointer-events-none">
                            <div className={clsx('text-[9px] font-black text-white px-1.5 py-0.5 rounded-md opacity-70', z.colour)}>{z.name.split('—')[0].trim()}</div>
                        </div>
                    ))}
                </div>
                {tapPos && (
...
function HeatmapTab({ imageUrl, zones, points }: { imageUrl: string; zones: FloorZone[]; points: LeadPoint[] }) {
    ...
    return (
        <div className="space-y-5">
            ...
            <div ref={containerRef} className="relative rounded-2xl overflow-hidden border border-psi bg-slate-900" style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '55vh' }}>
                        ...
                        return (
                            <motion.div key={z.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    <div className={clsx('w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black flex-shrink-0 text-white', z.colour)}>{i === 0 ? '🏆' : `#${i + 1}`}</div>

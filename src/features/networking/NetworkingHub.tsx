...
function getScoreColor(score: number) {
    ...
    return 'text-psi-secondary';
}

function getScoreBg(score: number) {
    if (score >= 80) return 'bg-emerald-500/20 border-emerald-500/30';
    if (score >= 60) return 'bg-amber-500/20 border-amber-500/30';
    return 'bg-psi-subtle border-psi-strong';
}
    ...
    return (
        <div className="relative w-14 h-14 flex-shrink-0">
            <svg className="w-14 h-14 -rotate-90" viewBox="0 0 44 44">
                <circle cx="22" cy="22" r={r} fill="none" strokeWidth="3" className="stroke-psi-border" />
                <circle
                    cx="22" cy="22" r={r} fill="none" strokeWidth="3"
                    strokeDasharray={circ}
                    strokeDashoffset={offset}
                    strokeLinecap="round"
                    className={score >= 80 ? 'stroke-emerald-400' : score >= 60 ? 'stroke-amber-400' : 'stroke-psi-border'}
...
function MatchCard({
    ...
    return (
        <motion.div
            ...
            className="bg-psi-surface border border-psi rounded-2xl overflow-hidden hover:border-psi-strong transition-colors"
        >
            {/* Card Header */}
            <div className="p-5 flex gap-4">
                {/* Avatar */}
                <div className={cn(
                    'w-14 h-14 rounded-2xl bg-gradient-to-br flex items-center justify-center flex-shrink-0 text-psi-primary font-black text-lg shadow-lg',
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                        <div>
                            <div className="flex items-center gap-2">
                                <h3 className="text-psi-primary font-bold text-base leading-tight">{attendee.name}</h3>
                                {attendee.isOnline && (
                                    <span className="w-2 h-2 rounded-full bg-emerald-400 flex-shrink-0" title="Online now" />
                                )}
                            </div>
                            <p className="text-psi-secondary text-xs mt-0.5 leading-tight">{attendee.jobTitle}</p>
                            <p className="text-psi-secondary text-xs leading-tight">{attendee.company}</p>
                        </div>
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                        ...
                        <span className="flex items-center gap-1 text-[11px] text-psi-secondary">
            ...
            <div className="px-5 pb-3">
                <div className="bg-psi-subtle/60 rounded-xl p-3 flex gap-2">
                    <Sparkles size={13} className="text-amber-400 flex-shrink-0 mt-0.5" />
                    <p className="text-psi-secondary text-xs leading-relaxed">{rationale}</p>
                </div>
            </div>
            ...
            {sharedInterests.length > 0 && (
                <div className="px-5 pb-3 flex flex-wrap gap-1.5">
                    ...
                    {sharedInterests.length > 4 && (
                        <span className="text-[10px] text-psi-secondary">+{sharedInterests.length - 4} more</span>
                    )}
                </div>
            )}
            ...
            {sharedSessions.length > 0 && (
                <div className="px-5 pb-3">
                    <p className="text-[10px] text-psi-secondary font-medium mb-1.5">ATTENDED TOGETHER</p>
                ...
                {alreadyConnected ? (
                    <>
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-sm font-medium hover:bg-psi-subtle transition-colors"
                        >
            ...
            <AnimatePresence>
                {expanded && !alreadyConnected && !requestPending && (
                    ...
                    >
                        <div className="px-5 pb-5 border-t border-psi pt-4 space-y-3">
                            <p className="text-psi-secondary text-xs">Add a personal note to your request:</p>
                            <textarea
                                value={msg}
                                ...
                                className="w-full bg-psi-subtle border border-psi-strong rounded-xl px-3 py-2.5 text-sm text-psi-primary placeholder-slate-500 resize-none focus:outline-none focus:border-emerald-500 transition-colors"
                            />
                            <div className="flex gap-2">
                                <button
                                    onClick={handleConnect}
                                    disabled={connecting}
                                    className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-psi-primary text-sm font-semibold transition-colors"
                                >
                                    {connecting ? <RefreshCw size={13} className="animate-spin" /> : <Send size={13} />}
                                    {connecting ? 'Sending…' : 'Send Request'}
                                </button>
                                <button onClick={() => setExpanded(false)} className="px-4 py-2 rounded-xl bg-psi-subtle text-psi-secondary text-sm hover:bg-psi-subtle transition-colors">
                                    Cancel
                                </button>
                            </div>
                        </div>
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="w-full max-w-md bg-psi-surface border border-psi-strong rounded-3xl overflow-hidden shadow-2xl"
            >
                {/* Header */}
                <div className="p-6 border-b border-psi">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-psi-primary font-bold text-lg">Schedule Meeting</h2>
                        <button onClick={onClose} className="p-2 rounded-xl bg-psi-subtle text-psi-secondary hover:text-psi-primary transition-colors"><X size={16} /></button>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className={cn('w-10 h-10 rounded-xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-bold text-sm', avatarColor(attendee.id))}>
                            {attendee.avatar}
                        </div>
                        <div>
                            <p className="text-psi-primary font-semibold text-sm">{attendee.name}</p>
                            <p className="text-psi-secondary text-xs">{attendee.jobTitle} · {attendee.company}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 space-y-5">
                    {/* Time slots */}
                    <div>
                        <label className="text-xs font-bold uppercase tracking-widest text-psi-secondary mb-2 block">Select Time Slot</label>
                    ...
                    {/* Location */}
                    <div>
                        <label className="text-xs font-bold uppercase tracking-widest text-psi-secondary mb-2 block">Meeting Location</label>
                    ...
                    {/* Notes */}
                    <div>
                        <label className="text-xs font-bold uppercase tracking-widest text-psi-secondary mb-2 block">Notes (optional)</label>
                        <textarea
                            value={notes}
                    ...
                    <button
                        ...
                        disabled={submitting}
                        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-psi-primary font-bold transition-colors"
                    >
...
export default function NetworkingHub() {
    ...
    return (
        <div className="min-h-screen bg-psi-page font-sans">
            ...
            {/* ── Page Header ─────────────────────────────────────────────── */}
            <div className="bg-psi-surface border-b border-psi px-6 py-5">
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <Network size={20} className="text-emerald-400" />
                                <h1 className="text-psi-primary font-extrabold text-xl">Smart Networking</h1>
                                {networkingEnabled
                            ...
                            <p className="text-psi-secondary text-sm">AI-powered attendee matchmaking · {allMatches.length} recommended connections</p>
                        </div>
                        {/* Stats row */}
                        <div className="flex gap-4">
                            {[
                                ...
                                <div key={s.label} className="text-center">
                                    ...
                                    <p className="text-psi-primary font-black text-base leading-none">{s.value}</p>
                                    <p className="text-psi-secondary text-[10px] font-medium uppercase tracking-wider">{s.label}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Tabs */}
                    <div className="flex gap-1 mt-5 bg-psi-subtle/60 p-1 rounded-xl w-fit">
                        {TABS.map(tab => {
                            const badge =
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {badge > 0 && (
                                        <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 text-psi-primary text-[9px] font-black flex items-center justify-center">
                                            {badge}
                                        </span>
                                    )}
                                </button>
                ...
                <AnimatePresence mode="wait">
                        ...
                        <motion.div key="discover" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                                <div className="relative flex-1 max-w-sm">
                                    <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-psi-secondary" />
                                    <input
                                        value={search}
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-psi-secondary text-xs whitespace-nowrap">Min match:</span>
                                    {[0, 40, 60, 75].map(v => (
                                        <button
                                            key={v}
                                            ...
                                            className={cn(
                                                ...
                                                filterMin === v
                                                    ? 'bg-emerald-600 border-emerald-500 text-white'
                                                    : 'bg-psi-surface border-psi-strong text-psi-secondary hover:border-psi-strong'
                                            )}
                                        >
                                            {v === 0 ? 'All' : `${v}%+`}
                                        </button>
                            ...
                            <div className="mb-6 bg-gradient-to-r from-emerald-900/40 to-teal-900/20 border border-emerald-800/40 rounded-2xl p-4 flex items-center gap-4">
                                <div className={cn('w-12 h-12 rounded-2xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-black text-base', avatarColor(ME.id))}>
                                    {ME.avatar}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-psi-primary font-bold text-sm">Matching as: {ME.name}</p>
                                    <p className="text-psi-secondary text-xs">{ME.jobTitle} · {ME.company} · {ME.industry}</p>
                                ...
                                <div className="flex-shrink-0 text-right">
                                    ...
                                    <p className="text-psi-secondary text-[10px] uppercase tracking-wider">matches found</p>
                                </div>
                            </div>

                            {/* Match grid */}
                            {filteredMatches.length === 0 ? (
                                <div className="flex flex-col items-center py-24 bg-psi-surface/60 border-2 border-dashed border-psi rounded-3xl text-center">
                                    <Users size={40} className="text-psi-muted mb-4" />
                                    <h3 className="text-psi-primary font-bold text-lg mb-1">No matches found</h3>
                                    <p className="text-psi-secondary text-sm">Try lowering the minimum match threshold or clearing the search</p>
                                </div>
                            ) : (
                    ...
                    {activeTab === 'network' && (
                        <motion.div key="network" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                            {acceptedIds.size === 0 ? (
                                <div className="flex flex-col items-center py-24 bg-psi-surface/60 border-2 border-dashed border-psi rounded-3xl text-center">
                                    <UserCheck size={40} className="text-psi-muted mb-4" />
                                    <h3 className="text-psi-primary font-bold text-lg mb-2">No connections yet</h3>
                                    <p className="text-psi-secondary text-sm mb-6">Go to Discover and send your first connection request</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                                    {allMatches.filter(r => acceptedIds.has(r.attendee.id)).map(result => (
                                        <div key={result.attendee.id} className="bg-psi-surface border border-psi rounded-2xl p-5">
                                            <div className="flex items-center gap-4 mb-4">
                                                <div className={cn('w-12 h-12 rounded-2xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-black', avatarColor(result.attendee.id))}>
                                                    {result.attendee.avatar}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-psi-primary font-bold text-sm">{result.attendee.name}</p>
                                                    <p className="text-psi-secondary text-xs">{result.attendee.jobTitle}</p>
                                                    <p className="text-psi-secondary text-xs">{result.attendee.company}</p>
                                                </div>
                                            ...
                                            <button
                                                ...
                                                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-sm font-medium hover:bg-psi-subtle transition-colors"
                                            >
                    ...
                    {activeTab === 'requests' && (
                        <motion.div key="requests" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
                            {/* Incoming */}
                            <section>
                                <h2 className="text-psi-primary font-bold text-base mb-4 flex items-center gap-2">
                                ...
                                {incomingPending.length === 0 ? (
                                    <div className="bg-psi-surface border border-psi rounded-2xl p-8 text-center text-psi-secondary text-sm">No incoming requests</div>
                                ) : (
                                    <div className="space-y-3">
                                        {incomingPending.map(req => {
                                            const sender = DEMO_ATTENDEES.find(a => a.id === req.fromId);
                                            return (
                                                <div key={req.id} className="bg-psi-surface border border-psi rounded-2xl p-5">
                                                    <div className="flex items-start gap-4">
                                                        {sender && (
                                                            <div className={cn('w-11 h-11 rounded-xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-black text-sm flex-shrink-0', avatarColor(sender.id))}>
                                                                {sender.avatar}
                                                            </div>
                                                        )}
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-psi-primary font-bold text-sm">{req.fromName}</p>
                                                            <p className="text-psi-secondary text-xs mb-2">{req.fromTitle}</p>
                                                            <div className="bg-psi-subtle rounded-xl p-3 mb-3">
                                                                <p className="text-psi-secondary text-xs italic">"{req.message}"</p>
                                                            </div>
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button onClick={() => respondToRequest(req.id, false)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-xs font-medium hover:bg-psi-subtle transition-colors">
                                                                    <X size={12} /> Decline
                                                                </button>
                                                            </div>
                                                        </div>
                                                        <p className="text-psi-muted text-[10px] flex-shrink-0">
                            ...
                            <section>
                                <h2 className="text-psi-primary font-bold text-base mb-4 flex items-center gap-2">
                                    <Send size={14} className="text-psi-secondary" />
                                    Sent Requests
                                </h2>
                                {outgoingPending.length === 0 ? (
                                    <div className="bg-psi-surface border border-psi rounded-2xl p-8 text-center text-psi-secondary text-sm">No outgoing requests</div>
                                ) : (
                                    <div className="space-y-3">
                                        {outgoingPending.map(req => {
                                            const target = DEMO_ATTENDEES.find(a => a.id === req.toId);
                                            return (
                                                <div key={req.id} className="bg-psi-surface border border-psi rounded-2xl p-5 flex items-center gap-4">
                                                    {target && (
                                                        <div className={cn('w-11 h-11 rounded-xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-black text-sm flex-shrink-0', avatarColor(target.id))}>
                                                            {target.avatar}
                                                        </div>
                                                    )}
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-psi-primary font-bold text-sm">{req.toName}</p>
                                                        <p className="text-amber-400 text-xs flex items-center gap-1"><Clock size={10} /> Pending response</p>
                                                    </div>
                                                    <button onClick={() => withdrawRequest(req.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-psi-subtle border border-psi-strong text-psi-secondary text-xs hover:text-red-400 hover:border-red-800 transition-colors">
                                                        <X size={11} /> Withdraw
                                                    </button>
                                                </div>
                                            );
                    ...
                    {activeTab === 'meetings' && (
                        <motion.div key="meetings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-psi-primary font-bold text-base">Scheduled Meetings ({meetings.length})</h2>
                                <button
                            ...
                            {meetings.length === 0 ? (
                                <div className="flex flex-col items-center py-24 bg-psi-surface/60 border-2 border-dashed border-psi rounded-3xl text-center">
                                    <Coffee size={40} className="text-psi-muted mb-4" />
                                    <h3 className="text-psi-primary font-bold text-lg mb-2">No meetings yet</h3>
                                    <p className="text-psi-secondary text-sm">Connect with attendees and schedule your first meeting</p>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    {meetings.map(m => (
                                        <div key={m.id} className="bg-psi-surface border border-psi rounded-2xl p-5 flex items-center gap-5">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-psi-primary font-bold text-sm">{m.attendeeBName}</p>
                                                <div className="flex items-center gap-3 mt-1">
                                                    <span className="flex items-center gap-1 text-psi-secondary text-xs"><Clock size={10} /> {m.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – {m.endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                    <span className="flex items-center gap-1 text-psi-secondary text-xs"><MapPin size={10} /> {m.location}</span>
                                                </div>
                                                {m.notes && <p className="text-psi-secondary text-xs mt-1 italic">"{m.notes}"</p>}
                                            </div>
                    ...
                    {activeTab === 'admin' && (
                        <motion.div key="admin" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
                            ...
                            <div className="bg-psi-surface border border-psi rounded-2xl p-6">
                                <h2 className="text-psi-primary font-bold text-base mb-1 flex items-center gap-2"><Shield size={16} className="text-emerald-400" /> Networking Control</h2>
                                <p className="text-psi-secondary text-sm mb-5">Enable or disable the networking system for this event. When disabled, attendees cannot discover or send connection requests.</p>
                                <div className="flex items-center justify-between p-4 bg-psi-subtle rounded-xl">
                                    <div>
                                        <p className="text-psi-primary font-semibold text-sm">Smart Networking</p>
                                        <p className="text-psi-secondary text-xs">AI matchmaking + requests + scheduling</p>
                                    </div>
                                    <button
                                        onClick={() => { setNetworkingEnabled(e => !e); toast.success(`Networking ${networkingEnabled ? 'disabled' : 'enabled'}`); }}
                                        className={cn(
                                            'relative w-12 h-6 rounded-full transition-colors duration-300',
                                            networkingEnabled ? 'bg-emerald-500' : 'bg-psi-border'
                                        )}
                                    >
                            ...
                            {/* Analytics */}
                            <div className="bg-psi-surface border border-psi rounded-2xl p-6">
                                <h2 className="text-psi-primary font-bold text-base mb-5 flex items-center gap-2"><BarChart3 size={16} className="text-emerald-400" /> Networking Analytics</h2>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {[
                                        ...
                                        <div key={s.label} className="bg-psi-subtle rounded-xl p-4 text-center">
                                            <s.icon size={20} className={`mx-auto mb-2 text-${s.color}-400`} />
                                            <p className="text-psi-primary font-black text-2xl">{s.value}</p>
                                            <p className="text-psi-secondary text-[10px] font-bold uppercase tracking-wider mt-0.5">{s.label}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {/* Score distribution */}
                            <div className="bg-psi-surface border border-psi rounded-2xl p-6">
                                <h2 className="text-psi-primary font-bold text-base mb-5 flex items-center gap-2"><TrendingUp size={16} className="text-emerald-400" /> Match Quality Distribution</h2>
                                <div className="space-y-3">
                                    {[
                                        ...
                                        { label: 'Potential Match (20–54%)', min: 20, color: 'bg-psi-subtle0' },
                                    ].map(band => {
                                        ...
                                        return (
                                            <div key={band.label} className="flex items-center gap-3">
                                                <p className="text-psi-secondary text-xs w-48 flex-shrink-0">{band.label}</p>
                                                <div className="flex-1 bg-psi-subtle rounded-full h-2">
                                                    <motion.div
                                                        initial={{ width: 0 }}
                                                        animate={{ width: `${pct}%` }}
                                                ...
                                                <span className="text-psi-secondary text-xs w-8 text-right">{count}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                            ...
                            {/* Attendee list */}
                            <div className="bg-psi-surface border border-psi rounded-2xl p-6">
                                <h2 className="text-psi-primary font-bold text-base mb-4 flex items-center gap-2"><Users size={16} className="text-emerald-400" /> All Attendees ({DEMO_ATTENDEES.length})</h2>
                                <div className="space-y-2">
                                    {DEMO_ATTENDEES.map(a => (
                                        <div key={a.id} className="flex items-center gap-3 py-2 border-b border-psi last:border-0">
                                            <div className={cn('w-8 h-8 rounded-xl bg-gradient-to-br flex items-center justify-center text-psi-primary font-black text-xs flex-shrink-0', avatarColor(a.id))}>
                                                {a.avatar}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-psi-primary text-sm font-semibold truncate">{a.name}</p>
                                                <p className="text-psi-secondary text-xs truncate">{a.jobTitle} · {a.company}</p>
                                            </div>
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                <span className={cn('w-1.5 h-1.5 rounded-full', a.isOnline ? 'bg-emerald-400' : 'bg-psi-border')} />

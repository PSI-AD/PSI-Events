...
function StatChip({ icon: Icon, value, label }: { icon: React.ElementType; value: number | string; label: string }) {
    return (
        <div className="flex flex-col items-center p-3 bg-psi-subtle rounded-xl">
            <Icon size={14} className="text-emerald-400 mb-1" />
            <p className="text-psi-primary font-black text-base">{typeof value === 'number' ? value.toLocaleString() : value}</p>
            <p className="text-psi-secondary text-[9px] uppercase tracking-wider">{label}</p>
        </div>
    );
}
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start justify-between gap-3 mb-4">
                <div className={cn('w-14 h-14 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0', 'bg-psi-subtle border border-psi-strong')}>
                    {exhibitor.logo}
                </div>
            ...
            <div className="mb-3">
                <h3 className="text-psi-primary font-black text-sm leading-tight mb-0.5">{exhibitor.name}</h3>
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <span className="text-[10px] text-psi-secondary flex items-center gap-0.5">
                    <MapPin size={9} /> Stand {exhibitor.standNumber}
                </span>
                <span className="text-[10px] text-psi-secondary">{exhibitor.country}</span>
            </div>

            {/* Tags */}
            <div className="flex flex-wrap gap-1 mb-4">
                {exhibitor.tags.slice(0, 3).map(t => (
                    <span key={t} className="text-[9px] bg-psi-subtle border border-psi-strong text-psi-secondary px-1.5 py-0.5 rounded-full">{t}</span>
                ))}
            </div>

            {/* Footer stats */}
            <div className="flex items-center justify-between pt-3 border-t border-psi">
                <div className="flex gap-3 text-psi-secondary text-xs">
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-psi-surface border border-psi-strong rounded-3xl p-6 w-full max-w-md shadow-2xl"
            >
                <div className="flex items-center gap-3 mb-5">
                    <div className="w-10 h-10 rounded-xl bg-psi-subtle flex items-center justify-center text-xl">{exhibitor.logo}</div>
                    <div>
                        <h3 className="text-psi-primary font-bold">{exhibitor.name}</h3>
                        <p className="text-psi-secondary text-xs">Stand {exhibitor.standNumber}</p>
                    </div>
                </div>

                {/* Type toggle */}
                <div className="flex bg-psi-subtle rounded-xl p-1 mb-4">
                    {(['Meeting', 'Demo'] as const).map(t => (
                        <button key={t} onClick={() => setType(t)}
                            className={cn('flex-1 py-2 rounded-lg text-xs font-bold transition-all', type === t ? 'bg-emerald-600 text-white' : 'text-psi-secondary hover:text-psi-primary')}>
                ...
                <div className="space-y-3 mb-4">
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name"
                        className="w-full bg-psi-subtle border border-psi-strong rounded-xl px-3 py-2.5 text-sm text-psi-primary placeholder-slate-500 focus:outline-none focus:border-emerald-500" />
                    <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Company / Organization"
                        className="w-full bg-psi-subtle border border-psi-strong rounded-xl px-3 py-2.5 text-sm text-psi-primary placeholder-slate-500 focus:outline-none focus:border-emerald-500" />
                </div>

                <p className="text-psi-secondary text-xs font-black uppercase tracking-wider mb-2">Select Time Slot</p>
                <div className="grid grid-cols-4 gap-1.5 mb-5">
                    {exhibitor.availableSlots.map(slot => (
                        <button key={slot} onClick={() => setSelectedSlot(slot)}
                            className={cn('py-2 rounded-lg text-xs font-bold border transition-all', selectedSlot === slot ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-psi-subtle border-psi-strong text-psi-secondary hover:border-psi-strong')}>
                            {slot}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={onClose} className="px-5 py-3 rounded-2xl bg-psi-subtle border border-psi-strong text-psi-secondary text-sm hover:text-psi-primary transition-colors">
                        Cancel
                    </button>
                </div>
            </motion.div>
        ...
        <>
            <div className="flex-1 overflow-y-auto">
                ...
                <div className={cn('bg-gradient-to-r p-6 pb-0 relative', exhibitor.color)}>
                    <button onClick={onBack} className="flex items-center gap-1.5 text-psi-secondary hover:text-psi-primary text-xs font-semibold mb-4 transition-colors">
                        <ChevronLeft size={14} /> Back to Directory
                    </button>
                    <div className="flex items-start gap-5 mb-5 flex-wrap">
                        <div className="w-20 h-20 rounded-3xl bg-psi-subtle/80 border border-psi-strong/60 flex items-center justify-center text-4xl flex-shrink-0 shadow-2xl">
                            {exhibitor.logo}
                        </div>
                        <div className="flex-1 min-w-0">
                            ...
                            <h1 className="text-psi-primary font-black text-2xl leading-tight">{exhibitor.name}</h1>
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                            ...
                            <button onClick={() => { setModalProduct(null); setShowModal(true); }}
                                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-psi-border-subtle hover:bg-psi-subtle text-psi-primary text-sm font-bold transition-colors border border-black/20 dark:border-white/20 backdrop-blur">
                        ...
                        {([
                            ...
                            <button key={t.id} onClick={() => setTab(t.id)}
                                className={cn('flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold transition-all',
                                    tab === t.id ? 'bg-psi-border text-psi-primary backdrop-blur' : 'text-psi-secondary hover:text-psi-primary/90')}>
                        ...
                        {tab === 'overview' && (
                            <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                                ...
                                <div className="bg-psi-surface border border-psi rounded-2xl p-5">
                                    <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-2">About</p>
                                    <p className="text-psi-secondary text-sm leading-relaxed">{exhibitor.description}</p>
                                </div>

                                {/* Key stats */}
                                <div>
                                    <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Engagement Stats</p>
                                ...
                                <div className="bg-psi-surface border border-psi rounded-2xl p-5 flex items-center gap-4">
                                    <div className="w-14 h-14 rounded-2xl bg-psi-subtle border border-psi-strong flex items-center justify-center text-3xl flex-shrink-0">
                                        {exhibitor.keyContact.avatar}
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-psi-primary font-bold">{exhibitor.keyContact.name}</p>
                                        <p className="text-psi-secondary text-sm">{exhibitor.keyContact.title}</p>
                                        <p className="text-psi-secondary text-xs mt-0.5">{exhibitor.name}</p>
                                    </div>
                                    <button onClick={() => setShowModal(true)}
                                ...
                                {exhibitor.sessions.length > 0 && (
                                    <div>
                                        <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Sessions & Presentations</p>
                                        {exhibitor.sessions.map((s, i) => (
                                            <div key={i} className="flex items-center gap-3 bg-psi-surface border border-psi rounded-xl p-3 mb-2">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-psi-primary text-sm font-semibold truncate">{s.sessionName}</p>
                                                    <p className="text-psi-secondary text-xs">{s.room} · {s.time}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {exhibitor.tags.map(t => (
                                        <span key={t} className="text-xs bg-psi-subtle border border-psi-strong text-psi-secondary px-2.5 py-1 rounded-full">{t}</span>
                                    ))}
                                </div>
                            </motion.div>
                        )}
                        ...
                        {tab === 'products' && (
                            <motion.div key="products" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                                ...
                                {exhibitor.products.map(prod => (
                                    <div key={prod.id} className="bg-psi-surface border border-psi hover:border-psi-strong rounded-2xl p-5 transition-colors">
                                        <div className="flex items-start gap-3 mb-3">
                                            <div className="w-12 h-12 rounded-xl bg-psi-subtle flex items-center justify-center text-2xl flex-shrink-0">{prod.imageEmoji}</div>
                                            <div className="flex-1 min-w-0">
                                                <span className="text-[9px] font-black uppercase tracking-wider text-psi-secondary">{prod.type}</span>
                                                <h3 className="text-psi-primary font-bold text-sm leading-tight">{prod.name}</h3>
                                                {prod.priceNote && <p className="text-emerald-400 text-xs font-bold mt-0.5">{prod.priceNote}</p>}
                                            </div>
                                        </div>
                                        <p className="text-psi-secondary text-sm leading-relaxed mb-3">{prod.description}</p>
                                        <div className="flex flex-wrap gap-1 mb-4">
                                            {prod.highlights.map(h => (
                                                <span key={h} className="flex items-center gap-0.5 text-[10px] text-psi-secondary bg-psi-subtle border border-psi-strong px-2 py-0.5 rounded-full">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button onClick={() => setShowModal(true)}
                                                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-xs font-semibold hover:bg-psi-subtle transition-colors">
                        ...
                        {tab === 'documents' && (
                            <motion.div key="documents" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                                ...
                                {exhibitor.documents.length === 0 ? (
                                    ...
                                    <div key={doc.id} className="flex items-center gap-4 bg-psi-surface border border-psi rounded-xl p-4 hover:border-psi-strong transition-colors">
                                        <div className="w-12 h-12 rounded-xl bg-psi-subtle flex items-center justify-center text-2xl flex-shrink-0">{doc.emoji}</div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-psi-primary font-semibold text-sm truncate">{doc.name}</p>
                                            <div className="flex items-center gap-3 mt-0.5">
                                                <span className="text-psi-secondary text-xs">{doc.type}</span>
                                                <span className="text-psi-muted text-xs">{doc.sizeLabel}</span>
                                                <span className="text-psi-muted text-xs flex items-center gap-0.5"><Download size={9} /> {doc.downloads.toLocaleString()} downloads</span>
                                            </div>
                                        </div>
                                        <button onClick={() => handleDownload(doc.name)}
                                            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-psi-subtle border border-psi-strong text-psi-secondary text-xs font-semibold hover:bg-emerald-600 hover:border-emerald-600 hover:text-white transition-all flex-shrink-0">
                        ...
                        {tab === 'contact' && (
                            <motion.div key="contact" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                                ...
                                <div className="space-y-4">
                                    <div className="bg-psi-surface border border-psi rounded-2xl p-5 space-y-3">
                                        <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider">Contact Details</p>
                                        {[
                                            ...
                                            <div key={label} className="flex items-center gap-3 text-sm">
                                                <Icon size={14} className="text-psi-secondary flex-shrink-0" />
                                                <span className="text-psi-secondary">{label}</span>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="bg-psi-surface border border-psi rounded-2xl p-5">
                                        <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Key Contact</p>
                                        <div className="flex items-center gap-3">
                                            <span className="text-3xl">{exhibitor.keyContact.avatar}</span>
                                            <div>
                                                <p className="text-psi-primary font-bold text-sm">{exhibitor.keyContact.name}</p>
                                                <p className="text-psi-secondary text-xs">{exhibitor.keyContact.title}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <button onClick={() => { setShowModal(true); }}
                                        className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-psi-surface border border-psi-strong text-psi-secondary font-semibold text-sm transition-colors hover:bg-psi-subtle">
                                        <Zap size={14} /> Request Product Demo
                                    </button>
                                    <button onClick={() => toast.success('Contact request sent!')}
                                        className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-psi-surface border border-psi-strong text-psi-secondary pointer text-sm transition-colors hover:bg-psi-subtle">
                                        <MessageSquare size={14} /> Send Message
                                    </button>
                                    <div className="bg-psi-surface border border-psi rounded-xl p-4 text-center">
                                        <Clock size={16} className="text-psi-muted mx-auto mb-1" />
...
function AdminPanel() {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
            {/* Global KPIs */}
            <div>
                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Platform-Wide Engagement</p>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {[
                        ...
                        <div key={s.label} className="bg-psi-surface border border-psi rounded-2xl p-4 text-center">
                            <s.icon size={18} className={cn('mx-auto mb-2', s.color)} />
                            <p className="text-psi-primary font-black text-2xl">{s.value.toLocaleString()}</p>
                            <p className="text-psi-secondary text-[9px] uppercase tracking-wider">{s.label}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {/* Leaderboard */}
            <div>
                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Exhibitor Leaderboard — By Profile Views</p>
                <div className="space-y-2">
                    {sorted.map((e, i) => {
                        const maxViews = sorted[0].analytics.profileViews;
                        const pct = Math.round((e.analytics.profileViews / maxViews) * 100);
                        return (
                            <div key={e.id} className="flex items-center gap-3 bg-psi-surface border border-psi rounded-xl p-3">
                                <span className={cn('font-black text-sm w-5 text-center', i === 0 ? 'text-amber-400' : i === 1 ? 'text-psi-secondary' : i === 2 ? 'text-orange-600' : 'text-psi-muted')}>{i + 1}</span>
                                <span className="text-xl w-7">{e.logo}</span>
                                <div className="flex-1 min-w-0">
                                    <p className="text-psi-primary text-xs font-semibold truncate">{e.name}</p>
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <div className="flex-1 bg-psi-subtle rounded-full h-1.5">
                                            <motion.div animate={{ width: `${pct}%` }} className="h-1.5 rounded-full bg-emerald-500" />
                                        </div>
                                        <span className="text-psi-secondary text-[10px] font-bold">{e.analytics.profileViews.toLocaleString()} views</span>
                                    </div>
                                </div>
            ...
            {/* Management table */}
            <div>
                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">Exhibitor Management</p>
                <div className="bg-psi-surface border border-psi rounded-2xl overflow-hidden">
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-psi">
                                {['Exhibitor', 'Tier', 'Stand', 'Contacts', 'Demos', 'Visible', 'Featured'].map(h => (
                                    <th key={h} className="text-left text-psi-secondary text-[10px] font-black uppercase tracking-wider px-4 py-3">{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {exhibitors.map(e => (
                                <tr key={e.id} className={cn('border-b border-psi/50 hover:bg-psi-subtle/30 transition-colors', !e.isVisible && 'opacity-40')}>
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            <span className="text-lg">{e.logo}</span>
                                            <span className="text-psi-primary text-xs font-semibold truncate max-w-36">{e.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3"><TierBadge tier={e.tier} /></td>
                                    <td className="px-4 py-3 text-psi-secondary text-xs">{e.standNumber}</td>
                                    <td className="px-4 py-3 text-psi-secondary text-xs font-bold">{e.analytics.contactRequests}</td>
                                    <td className="px-4 py-3 text-psi-secondary text-xs font-bold">{e.analytics.demoBookings}</td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3">
                                        <button onClick={() => toggleFeatured(e.id)}
                                            className={cn('flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold border transition-all',
                                                e.isFeatured ? 'bg-amber-500/15 text-amber-400 border-amber-500/20' : 'bg-psi-border text-psi-secondary border-psi-strong')}>
...
export default function Marketplace() {
    ...
    return (
        <div className="h-screen bg-psi-page flex flex-col font-sans overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-psi-primary font-extrabold text-base">Sponsor & Exhibitor Marketplace</h1>
                        </div>
                        ...
                        {(['directory', 'admin'] as const).map(v => (
                            <button key={v} onClick={() => { setView(v); setSelectedExhibitor(null); }}
                                className={cn('flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold border transition-all',
                                    view === v ? (v === 'admin' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-amber-600 border-amber-500 text-white') : 'bg-psi-subtle border-psi-strong text-psi-secondary hover:text-psi-primary')}>
            ...
            {/* ── Directory filters ──────────────────────────────────────── */}
            {view === 'directory' && (
                <div className="border-b border-psi px-5 py-3 bg-psi-surface/40 flex-shrink-0">
                    <div className="flex items-center gap-3 flex-wrap">
                        <div className="relative">
                            <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-psi-secondary" />
                            <input value={query} onChange={e => setQuery(e.target.value)}
                                placeholder="Search exhibitors…"
                                className="bg-psi-subtle border border-psi-strong rounded-xl pl-8 pr-3 py-2 text-xs text-psi-primary placeholder-slate-500 focus:outline-none focus:border-amber-500 w-52 transition-colors" />
                        </div>
                        <div className="flex gap-1">
                            {(['All', ...TIERS] as const).map(t => (
                                <button key={t} onClick={() => setTierFilter(t)}
                                    className={cn('px-2.5 py-1.5 rounded-lg text-[10px] font-bold border transition-all',
                                        tierFilter === t ? 'bg-amber-600 border-amber-500 text-white' : 'bg-psi-subtle border-psi-strong text-psi-secondary hover:border-psi-strong')}>
                                    {t}
                                </button>
                            ))}
                        </div>
                        <div className="flex gap-1 flex-wrap">
                            {(['All', ...CATEGORIES] as const).map(c => (
                                <button key={c} onClick={() => setCatFilter(c)}
                                    className={cn('px-2.5 py-1.5 rounded-lg text-[10px] font-bold border transition-all',
                                        catFilter === c ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-psi-subtle border-psi-strong text-psi-secondary hover:border-psi-strong')}>
                                    {c}
                                </button>
                            ))}
                        </div>
                        <span className="text-psi-muted text-xs ml-auto hidden md:inline">{sorted.length} exhibitors</span>
                    </div>
                </div>
            )}
                ...
                <AnimatePresence mode="wait">
                    ...
                    {view === 'directory' && (
                        <motion.div key="dir" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            {/* All exhibitors */}
                            <div>
                                <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-3">All Exhibitors</p>
                                {sorted.length === 0 ? (
                                    <div className="text-center py-20 text-psi-muted">

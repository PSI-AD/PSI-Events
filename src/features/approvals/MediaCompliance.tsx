    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-slate-900 border border-rose-500/30 rounded-3xl p-6 max-w-sm w-full shadow-2xl"
            ...
            >
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <p className="font-extrabold text-white text-sm">Request Explanation</p>
                        <p className="text-[11px] text-white/30">{event.name}</p>
                    </div>
                </div>
                ...
                {!sent ? (
                    <>
                        <p className="text-sm text-white/70 leading-relaxed mb-4">
                            ...
                            <span className="text-white font-bold">{officerName ?? 'the assigned Media Officer'}</span>{' '}
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={onClose}
                                className="flex-1 py-2.5 rounded-xl bg-slate-800 text-white/60 text-sm font-bold hover:bg-slate-700 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                onClick={handleSend}
                                className="flex-1 py-2.5 rounded-xl bg-rose-500 text-white text-sm font-bold hover:bg-rose-400 transition-colors flex items-center justify-center gap-1.5"
                            >
                                <MailWarning size={14} />
                                Send Request
                            </button>
                    ...
                    >
                        ...
                        <p className="font-bold text-white text-sm">Request Sent</p>
                    ...
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                                <p className="font-extrabold text-white text-sm truncate">{record.event.name}</p>
                            ...
                            <p className="text-[11px] text-white/30">
                                ...
                                {' · '}
                                {record.isEventOver
                                    ? <span className="text-white/20">Event ended {new Date(record.event.end_date).toLocaleDateString('en-AE', { day: 'numeric', month: 'short' })}</span>
                        ...
                        <div className="flex-shrink-0 text-right">
                            {record.isLoading ? (
                                ...
                                <>
                                    ...
                                    <p className="text-[10px] text-white/25 font-bold">posts</p>
                                </>
                            )}
                        </div>
                    </div>
...
function InfoRow({
    ...
    return (
        <div className="flex items-start justify-between gap-3">
            <span className="text-[10px] font-bold text-white/25 uppercase tracking-wider flex-shrink-0 pt-px">{label}</span>
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h2 className="font-extrabold text-white text-base">Media Officer Compliance</h2>
                    <p className="text-[11px] text-white/30">Journal coverage tracker per event</p>
                </div>
            </div>
                ...
                {[
                    ...
                    <button
                        ...
                        className={`rounded-2xl border p-3 text-center transition-all active:scale-[0.97]
                            ${filterStatus === filter ? `${bg} ${border}` : 'bg-white/3 border-white/8 hover:bg-white/5'}
                        `}
                    >
                        <p className={`text-2xl font-extrabold font-mono ${color}`}>{count}</p>
                        <p className={`text-[10px] font-bold uppercase tracking-wider mt-0.5 ${filterStatus === filter ? color : 'text-white/30'}`}>
                            {label}
                        </p>
                    </button>
                ))}
            ...
            {records.length === 0 && (
                <div className="text-center py-10 text-slate-600">
                    ...
                    <p className="text-sm font-bold text-slate-500">No events loaded</p>

...
const TIER_LABELS: Record<AgentTier, { label: string; color: string; bg: string; ring: string }> = {
    ...
    silver: { label: 'Silver', color: 'text-slate-300', bg: 'bg-slate-500/15', ring: 'ring-slate-400/25' },
...
function KPITile({ label, value, sub, icon, accent }: {
    ...
    return (
        <div className={`rounded-2xl p-4 ring-1 ${colors[accent]}`}>
            ...
            <p className="text-white font-extrabold text-xl font-mono">{value}</p>
            <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mt-0.5">{label}</p>
...
export function AgentAdvanceView({
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-white/8 p-6">
                {/* Top accent bar */}
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <p className="text-4xl font-extrabold text-white font-mono mb-1">{fmt(lockedCommission)}</p>
                        <p className="text-white/30 text-xs">{eventName} · Pending final settlement</p>
                        <div className="mt-2">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-right">
                        <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest mb-1">Max Advance (50%)</p>
                        <p className="text-emerald-400 font-extrabold text-2xl font-mono">{fmt(maxAdvance)}</p>
                        <p className="text-white/20 text-[9px] mt-0.5">After 2% fee: {fmt(Math.round(maxAdvance * 0.98))}</p>
                    </div>
                </div>
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p className="text-white font-extrabold mt-2">{fmt(existingRequest.requestedAmount)} advance requested</p>
                            <p className="text-white/40 text-xs mt-0.5">
                                Fee: {fmt(existingRequest.feeAed)} · Net draw: {fmt(existingRequest.netDrawAed)}
                            </p>
                            <p className="text-white/25 text-[10px] mt-1.5">{timeAgo(existingRequest.requestedAt)}</p>
            ...
            <AnimatePresence>
                {showForm && (
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                        className="rounded-3xl bg-white/4 border border-white/10 p-6 space-y-6">

                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Zap size={14} className="text-emerald-400" />
                                <p className="text-white font-extrabold text-sm">Configure Advance Request</p>
                            </div>
                            <button onClick={() => setShowForm(false)}
                                className="w-7 h-7 rounded-lg bg-white/6 border border-white/10 flex items-center justify-center text-white/40 hover:text-white transition-all">
                                <X size={12} />
                            </button>
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-3">
                                <span className="text-white/50 text-xs font-bold">Advance Amount</span>
                                <span className="text-white font-extrabold text-lg font-mono">{fmt(requestedAmount)}</span>
                            </div>
                            <input
                                id="advance-slider"
                                type="range"
                        ...
                        <div className="space-y-2">
                            {[
                                { label: 'Advance Requested', value: fmt(requestedAmount), color: 'text-white' },
                                ...
                                { label: 'Held to Settlement', value: fmt(lockedCommission - requestedAmount), color: 'text-white/40' },
            ...
            {!showForm && existingRequest && existingRequest.status === 'declined' && (
                <motion.button
                    ...
                    className="w-full flex items-center justify-center gap-2.5 py-4 rounded-2xl bg-white/6 border border-white/15 text-white/80 font-bold text-sm transition-all hover:bg-white/10"
                >
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <span className="text-white font-extrabold">{req.agentName}</span>
                                                <TierBadge tier={req.agentTier} />
                                            </div>
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <Building2 size={10} className="text-white/25" />
                                                <span className="text-white/35 text-[10px]">{req.agentBranch}</span>
                                                <span className="text-white/15">·</span>
                                                <Clock size={10} className="text-white/25" />
                                                <span className="text-white/35 text-[10px]">{timeAgo(req.requestedAt)}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="px-5 pb-4">
                                    <div className="grid grid-cols-3 gap-3 mb-4">
                                        {[
                                            { label: 'Locked Commission', value: fmt(req.lockedCommission), color: 'text-white/80' },
                                            ...
                                            <div key={label} className="bg-white/4 rounded-xl p-3 border border-white/6">
                                                ...
                                                <p className="text-white/25 text-[9px] mt-0.5 leading-tight">{label}</p>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between px-3 py-2 bg-white/3 border border-white/6 rounded-xl mb-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-white/40 text-xs">Advance Fee (2%)</span>
                                        </div>
                                    ...
                                    {declineId === req.id && (
                                        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
                                            ...
                                            <textarea
                                                ...
                                                className="w-full bg-white/5 border border-rose-500/30 rounded-xl px-3 py-2.5 text-white/80 text-xs leading-relaxed focus:outline-none focus:border-rose-500/60 resize-none"
                                            />
                                        </motion.div>
                                    )}
                                            ...
                                            {declineId === req.id ? (
                                                <>
                                                    ...
                                                    <button onClick={() => setDeclineId(null)}
                                                        className="px-4 rounded-xl bg-white/6 border border-white/10 text-white/50 hover:text-white transition-all">
                                                        Cancel
                                                    </button>
                                                </>
                                            ) : (
        ...
        <div className="min-h-screen bg-slate-950 text-white">
            ...
            <header className="border-b border-white/8 px-5 py-4">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <h1 className="text-white text-xl font-extrabold">Commission Advance</h1>
                        <p className="text-white/30 text-xs mt-0.5">{eventName} · Advance at 2% fee · 50% cap</p>
                    </div>

        ...
        >
            ...
            <motion.div
                ...
                className={cn(
                    ...
                    isVoted ? 'bg-emerald-600/25' : 'bg-slate-700/40'
                )}
            />
            {/* Content */}
            <div className="relative flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                    ...
                    <span className="text-slate-500 text-xs">({option.votes})</span>
                </div>
            </div>
        </button>
    );
        ...
        >
            ...
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                    <span className="text-slate-500 text-[11px]">{question.author}</span>
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {isHost && onModerate && (
                        <>
                            <span className="text-slate-500 text-xs flex items-center gap-1 mr-1">
...
function AttendeeView({
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
            {/* Tab bar */}
            <div className="flex border-b border-slate-800 px-4 bg-slate-900/60">
                {([
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    <>
                        {livePoll ? (
                            <div>
                                ...
                                <h3 className="text-white font-bold text-base mb-4 leading-snug">{livePoll.question}</h3>
                                ...
                                <div className="flex items-center justify-between mt-3">
                                    <p className="text-slate-500 text-xs">{livePoll.totalVotes} votes · Live updating</p>
                            ...
                            <div className="text-center py-16">
                                ...
                                <p className="text-white font-bold mb-1">No active poll</p>
                                <p className="text-slate-500 text-sm">The host will launch a poll soon</p>
                            </div>
                        )}
                    </>
                )}
                    ...
                    <>
                        ...
                        {highlightedQuestion && (
                            <div className="bg-amber-900/20 border border-amber-600/40 rounded-2xl p-4 mb-2">
                                ...
                                <p className="text-white font-semibold text-sm leading-snug">{highlightedQuestion.text}</p>
                            </div>
                        )}

                        {/* Submit question */}
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
                            <p className="text-white font-semibold text-sm mb-3">Submit a Question</p>
                            {submitted ? (
                                ...
                                <>
                                    <textarea
                                        ...
                                        rows={3}
                                        maxLength={280}
                                        className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 resize-none transition-colors mb-2"
                                    />
                                    <div className="flex items-center gap-2">
                                        <input
                                            value={qAuthor}
                                            ...
                                            className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors"
                                        />
                                        <button
                        ...
                        <div>
                            <p className="text-slate-500 text-[10px] font-black uppercase tracking-wider mb-2">{approvedQuestions.length} Approved Questions</p>
                            <AnimatePresence>
                ...
                {tab === 'reactions' && (
                    <div>
                        <p className="text-white font-bold text-sm mb-4">Tap to send a reaction</p>
                        <div className="grid grid-cols-4 gap-3 mb-6">
                            {EMOJI_OPTIONS.map(emoji => (
                                <button
                                    key={emoji}
                                    onClick={() => { sendReaction(emoji); toast.success('Reaction sent!', { duration: 1000 }); }}
                                    className="flex flex-col items-center gap-1.5 py-4 rounded-2xl bg-slate-800 border border-slate-700 hover:bg-slate-700 hover:scale-105 active:scale-95 transition-all"
                                >
                                    <span className="text-3xl">{emoji}</span>
                                    <span className="text-slate-400 text-[10px] font-bold">{(reactionCounts[emoji] ?? 0).toLocaleString()}</span>
                                </button>
                            ))}
                        </div>
                        <p className="text-slate-500 text-xs text-center">Reactions are visible to everyone in the room</p>
                    </div>
                )}
            </div>
        </div>
...
function HostView(props: ReturnType<typeof useEngagement>) {
    ...
    return (
        <div className="flex-1 flex overflow-hidden">
            ...
            {/* ── Left: Controls ──────────────────────────────────────── */}
            <div className="w-96 border-r border-slate-800 flex flex-col overflow-hidden flex-shrink-0">
                {/* Tab bar */}
                <div className="flex border-b border-slate-800 px-2 pt-1 bg-slate-900/60">
                    {([
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                        ...
                        <>
                            ...
                            <AnimatePresence>
                                {showNewPoll && (
                                    <motion.div
                                        ...
                                        className="bg-slate-800 border border-slate-700 rounded-2xl p-4 space-y-2 overflow-hidden"
                                    >
                                        <p className="text-white font-bold text-xs mb-2">Create Poll</p>
                                        ...
                                        <div className="flex gap-2 pt-1">
                                            ...
                                            <button onClick={() => setShowNewPoll(false)} className="px-4 py-2 rounded-xl bg-slate-700 text-slate-300 text-xs hover:bg-slate-600 transition-colors">Cancel</button>
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            {polls.map(poll => (
                                <div key={poll.id} className={cn(
                                    ...
                                    poll.isLive ? 'border-emerald-600/50' : 'border-slate-800'
                                )}>
                                    <div className="flex items-start gap-2 mb-2">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-white text-xs font-semibold leading-snug line-clamp-2">{poll.question}</p>
                                        </div>
                                    </div>
                                                ...
                                                return (
                                                    <div key={opt.id}>
                                                        <div className="flex justify-between text-[10px] mb-0.5">
                                                            <span className="text-slate-300 truncate">{opt.text}</span>
                    ...
                    {tab === 'qa' && (
                        <>
                            ...
                            <div>
                                <p className="text-slate-500 text-[10px] font-black uppercase tracking-wider mb-2">Approved ({approvedQuestions.length})</p>
                                <AnimatePresence>
                                    {approvedQuestions.map(q => (
                                        <div key={q.id} className="mb-2">
                                            <QuestionCard question={q} isHost onModerate={moderateQuestion} />
                            ...
                            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-center gap-4">
                                <div className="relative w-16 h-16 flex-shrink-0">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <span className="text-white font-black text-sm">{selectedSession.engagementScore}%</span>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-white font-bold">Engagement Score</p>
                                    <p className="text-slate-400 text-xs mt-0.5"><Users size={10} className="inline mr-1" />{selectedSession.attendeeCount} in session</p>
                                </div>
                            </div>

                            {/* Reaction breakdown */}
                            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4">
                                <p className="text-white font-bold text-xs mb-3">Reaction Totals</p>
                                {EMOJI_OPTIONS.map(emoji => {
                                    ...
                                    return (
                                        <div key={emoji} className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-slate-300 text-xs font-bold w-8 text-right">{count}</span>
                                        </div>
                                    );
                                })}
                            </div>
            ...
            <div className="flex-1 flex flex-col bg-slate-950 relative overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800 bg-slate-900/60 flex-shrink-0">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    {props.highlightedQuestion && (
                        <div className="bg-amber-900/25 border-2 border-amber-600/50 rounded-2xl p-6 text-center">
                            ...
                            <p className="text-white font-bold text-lg leading-snug">{props.highlightedQuestion.text}</p>
                            <p className="text-slate-400 text-sm mt-2">— {props.highlightedQuestion.author}</p>
                        </div>
                    )}
                    ...
                    {livePoll && (
                        <div className="bg-slate-900 border border-emerald-800/40 rounded-2xl p-6">
                            ...
                            <h3 className="text-white font-bold text-base mb-4">{livePoll.question}</h3>
                                    ...
                                    return (
                                        <div key={opt.id}>
                                            <div className="flex justify-between mb-1">
                                                <span className={cn('text-sm', isWinning ? 'text-white font-bold' : 'text-slate-300')}>{opt.text}</span>
                                                <span className={cn('font-black text-sm', isWinning ? 'text-emerald-400' : 'text-slate-400')}>{pct}%</span>
                                            </div>
                    ...
                    <div>
                        <p className="text-slate-500 text-xs font-black uppercase tracking-wider mb-3">Top Questions</p>
                        {approvedQuestions.slice(0, 3).map((q, i) => (
                            <div key={q.id} className={cn(
                                'mb-2 bg-slate-900 border rounded-xl p-3',
                                i === 0 ? 'border-violet-600/40' : 'border-slate-800'
                            )}>
                                <div className="flex items-start gap-2">
                                    ...
                                    <span className="text-slate-500 text-xs flex items-center gap-0.5 flex-shrink-0">
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
                        <p className="text-slate-500 text-xs font-black uppercase tracking-wider mb-3">Live Reactions</p>
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-white font-extrabold text-base">Live Engagement</h1>
                            <PulseDot />
                        </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <select
                            ...
                            className="bg-slate-800 border border-slate-700 text-slate-300 text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-emerald-500 transition-colors"
                        >
                            {SESSIONS.map(s => (
                        ...
                        <div className="flex bg-slate-800 border border-slate-700 rounded-xl p-0.5 gap-0.5">
                            <button
                                ...
                                className={cn(
                                    ...
                                    mode === 'attendee' ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
                                )}
                            >
                            ...
                            <button
                                ...
                                className={cn(
                                    'flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold transition-all',
                                    mode === 'host' ? 'bg-violet-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
                                )}
                            >
                                <Mic size={12} /> Host
                            </button>

...
function heatColor(v: number): string {
    if (v < 20) return 'bg-psi-subtle';
    if (v < 40) return 'bg-psi-border';
...
function KpiCard({ label, value, sub, icon: Icon, color, delay = 0 }: {
    ...
    return (
        <motion.div
            ...
            transition={{ delay }}
            className="bg-psi-surface border border-psi rounded-2xl p-4 flex items-center gap-4"
        >
            <div className={cn('w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0', color)}>
                <Icon size={18} className="text-psi-primary" />
            </div>
            <div>
                <p className="text-psi-primary font-black text-xl leading-none">{typeof value === 'number' ? value.toLocaleString() : value}</p>
        ...
        <div className="overflow-x-auto">
            ...
            <div className="flex items-center gap-2 mt-3 ml-24">
                ...
                <div className="flex gap-0.5">
                    {['bg-psi-subtle', 'bg-psi-border', 'bg-amber-900/70', 'bg-amber-700/80', 'bg-amber-500/80', 'bg-emerald-500/80', 'bg-emerald-400'].map(c => (
                        <div key={c} className={cn('w-6 h-2 rounded-sm', c)} />
                    ))}
                </div>
                <span className="text-slate-600 text-[9px]">High</span>
                {hovered && (
                    <span className="ml-4 text-psi-primary text-xs font-mono bg-psi-subtle px-2 py-0.5 rounded">{hovered.raw}</span>
                )}
            </div>
        </div>
    );
    ...
    return (
        ...
        >
            ...
            <button onClick={() => setExpanded(e => !e)}
                ...
                <div className="w-10 h-10 rounded-xl bg-psi-subtle border border-psi-strong flex items-center justify-center text-xl flex-shrink-0">
                    {insight.emoji}
                </div>
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-psi-primary font-bold text-sm leading-snug">{insight.title}</p>
                </div>
                <div className="flex-shrink-0 text-right">
                    <p className="text-psi-primary font-black text-lg leading-none">{insight.metric}</p>
                    <p className="text-psi-secondary text-[9px]">{insight.metricLabel}</p>
                </div>
            </button>
            ...
            <AnimatePresence>
                {expanded && (
                    ...
                    >
                        <div className="px-4 pb-4 space-y-3 border-t border-psi pt-3">
                            ...
                            {(revealed || displayedText.length > 30) && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
                                    ...
                                    <p className="text-psi-secondary text-xs leading-relaxed">{insight.recommendation}</p>
                                </motion.div>
                            )}
                        </div>
                    </motion.div>
...
function SpeakerRow({ s, rank }: { s: typeof SPEAKER_METRICS[0]; rank: number }) {
    return (
        <motion.div
            ...
            className="flex items-center gap-4 bg-psi-surface border border-psi rounded-xl p-3 hover:border-psi-strong transition-colors"
        >
            <div className="w-7 h-7 rounded-lg bg-psi-subtle flex items-center justify-center text-xs font-black text-psi-secondary flex-shrink-0">
                #{rank}
            </div>
            <div className="text-2xl flex-shrink-0">{s.avatar}</div>
            <div className="flex-1 min-w-0">
                <p className="text-psi-primary font-bold text-sm">{s.name}</p>
                <p className="text-psi-secondary text-xs truncate">{s.company}</p>
            </div>
            {/* Engagement bar */}
            <div className="w-32 flex-shrink-0">
                <div className="flex items-center justify-between mb-1">
                    <span className="text-slate-600 text-[9px]">Engagement</span>
                    <span className="text-psi-primary text-xs font-black">{s.avgEngagement}%</span>
                </div>
            ...
            <div className="flex items-center gap-1 flex-shrink-0 text-amber-400">
                ...
                <span className="text-psi-primary text-xs font-bold">{s.feedbackScore}</span>
            </div>
            <div className="text-psi-secondary text-xs flex-shrink-0">{s.totalReach.toLocaleString()} reached</div>
        </motion.div>
    );
}
...
function TopicBar({ t, rank }: { t: typeof TOPIC_SCORES[0]; rank: number }) {
    ...
    const trendColor = t.trend === 'rising' ? 'text-emerald-400' : t.trend === 'falling' ? 'text-red-400' : 'text-psi-secondary';
    ...
    return (
        ...
        >
            ...
            <span className="text-psi-primary text-xs w-52 truncate flex-shrink-0">{t.topic}</span>
            <div className="flex-1 h-5 bg-psi-subtle rounded-full overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
                ...
                >
                    <span className="text-psi-primary text-[9px] font-black">{t.score}</span>
                </motion.div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {NETWORKING_CLUSTERS.map((c, i) => (
                <motion.div
                    ...
                    className="bg-psi-surface border border-psi rounded-2xl p-4 hover:border-psi-strong transition-colors"
                >
                    <div className="flex items-start justify-between mb-3">
                        <div>
                            <p className="text-psi-primary font-bold text-sm">{c.label}</p>
                            <p className="text-psi-secondary text-xs">{c.connections} connections</p>
                        </div>
                        <div className={cn('w-10 h-10 rounded-full flex items-center justify-center text-sm font-black text-psi-primary flex-shrink-0', c.color)}>
                            {c.size}
                        </div>
                    </div>
                    {/* Size bar */}
                    <div className="h-1.5 bg-psi-subtle rounded-full overflow-hidden mb-3">
                        <motion.div
                            initial={{ width: 0 }}
                    ...
                    <div className="space-y-1">
                        {c.topPairs.map(p => (
                            <div key={p} className="flex items-center gap-1.5 text-psi-secondary text-[10px]">
...
export default function AIInsightsEngine() {
    ...
    return (
        <div className="h-screen bg-psi-page flex flex-col font-sans overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-psi-primary font-extrabold text-base">AI Event Insights Engine</h1>
                            {/* Scan status */}
                    ...
                    <div className="flex items-center gap-3 text-psi-secondary text-xs">
            ...
            {/* ── Tab bar ──────────────────────────────────────────────── */}
            <div className="flex border-b border-psi px-5 bg-psi-surface/40 flex-shrink-0 overflow-x-auto">
                {tabs.map(t => (
                    <button key={t.id} onClick={() => setTab(t.id)}
                        className={cn('flex items-center gap-1.5 px-4 py-3 text-sm font-semibold border-b-2 transition-colors flex-shrink-0',
                            tab === t.id ? 'border-violet-500 text-violet-400' : 'border-transparent text-psi-secondary hover:text-psi-primary')}>
                ...
                <AnimatePresence mode="wait">
                        ...
                        <motion.div key="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <div className="bg-psi-surface border border-psi rounded-2xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        <p className="text-psi-primary font-bold text-sm">Engagement Heatmap</p>
                                        <p className="text-psi-secondary text-xs">5 metrics across 13 hours · hover cells to inspect</p>
                                    </div>
                        ...
                        <motion.div key="act" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                ...
                                {([
                                    ...
                                    <button key={m.id} onClick={() => setChartMetric(m.id)}
                                        className={cn('px-3 py-1.5 rounded-xl border text-xs font-bold transition-all',
                                            chartMetric === m.id
                                                ? 'bg-psi-border border-slate-600 text-psi-primary'
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                {[
                                    ...
                                    <div key={s.label} className="bg-psi-surface border border-psi rounded-xl p-3 flex items-center gap-3">
                                        <s.icon size={16} className={s.color} />
                                        <div>
                                            <p className={cn('font-black text-base', s.color)}>{s.value}</p>
                                            <p className="text-psi-secondary text-xs">{s.label} · {s.time}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    {tab === 'speakers' && (
                        <motion.div key="sp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <p className="text-psi-secondary text-[10px] font-black uppercase tracking-wider mb-4">
                        ...
                        <motion.div key="tp" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <div className="bg-psi-surface border border-psi rounded-2xl p-5 space-y-3">
                                <div className="flex items-center justify-between mb-2">
                                    <p className="text-psi-primary font-bold text-sm">Topic Popularity Index</p>
                                    <div className="flex items-center gap-3 text-slate-600 text-[10px]">
                                        ...
                                        <span className="flex items-center gap-1"><Minus size={9} className="text-psi-secondary" /> Stable</span>
                        ...
                        <motion.div key="net" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    ...
                                    <div key={s.label} className="bg-psi-surface border border-psi rounded-xl p-3 text-center">
                                        <p className={cn('font-black text-xl', s.color)}>{s.value}</p>
                                        <p className="text-psi-secondary text-xs mt-0.5">{s.label}</p>
                                    </div>
                                ))}
                            </div>
                            <ClusterViz />
                            ...
                            {!scanComplete ? (
                                <div className="bg-violet-500/10 border border-violet-500/20 rounded-2xl p-4 flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="h-1 bg-psi-subtle rounded-full mt-2 overflow-hidden">
                                            <motion.div

...
const TIER_CFG: Record<VIPTier, { bg: string; text: string; border: string; badge: string; accent: string; qrFg: string; qrBg: string }> = {
    ...
    Gold: {
        bg: 'bg-gradient-to-br from-amber-950 to-slate-900',
        text: 'text-amber-100',
...
function ScannerFrame({ active }: { active: boolean }) {
    return (
        <div className="relative w-56 h-56 mx-auto">
            ...
            <div className="absolute inset-3 bg-slate-900/80 rounded-sm grid grid-cols-6 grid-rows-6 gap-px opacity-20">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                <div className={clsx('w-10 h-10 rounded-full border-2 flex items-center justify-center transition-colors', active ? 'border-amber-400/60' : 'border-slate-700')}>
                    ...
                    {DEMO_CONTACTS.map(c => (
                        <motion.button key={c.leadId} id={`contact-${c.leadId}`}
                            ...
                            <div className={clsx('w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all',
                                ...
                                {selected.has(c.leadId) && <CheckCircle2 size={13} className="text-white" />}
                            </div>
    ...
    return (
        <div className="space-y-5">
            ...
            <motion.div key={contact.leadId} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                ...
                {/* Ticket perforation */}
                <div className="relative bg-slate-900 h-6 flex items-center">
                    <div className="absolute -left-3 w-6 h-6 bg-psi-page rounded-full" />
                    <div className="flex-1 border-t-2 border-dashed border-slate-700 mx-3" />
                    <div className="absolute -right-3 w-6 h-6 bg-psi-page rounded-full" />
                </div>

                {/* Details strip */}
                <div className={clsx('px-6 pt-4 pb-5 space-y-3 bg-slate-900')}>
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-amber-500/20 flex items-center justify-center flex-shrink-0">
                            <User size={16} className="text-amber-400" />
                        </div>
                        <div>
                            <p className="text-white font-extrabold text-base leading-tight">{contact.name}</p>
                            <p className="text-slate-400 text-xs">Invited by {contact.assignedAgentName}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-800/60 rounded-xl px-3 py-2.5">
                            <p className="text-slate-500 text-[9px] font-bold uppercase tracking-widest">Event</p>
                            <p className="text-white text-xs font-bold leading-tight mt-0.5 truncate">{DEMO_EVENT.eventName}</p>
                        </div>
                        <div className="bg-slate-800/60 rounded-xl px-3 py-2.5">
                            <p className="text-slate-500 text-[9px] font-bold uppercase tracking-widest">Interest</p>
                            <p className="text-white text-xs font-bold leading-tight mt-0.5 truncate">{contact.projectInterest}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 pt-1">
                        <motion.button id="download-apple-wallet" whileTap={{ scale: 0.96 }}
                            ...
                            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-black/80 hover:bg-black text-white font-bold text-xs transition-all border border-slate-700">
        ...
        <div className="space-y-5">
            ...
            <div className="bg-slate-900 rounded-2xl p-5 border border-slate-800">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button id="toggle-vip-scanner"
                        ...
                        className={clsx('text-xs px-3 py-1.5 rounded-lg font-bold transition-colors',
                            scannerActive ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-slate-700 text-slate-400')}>
                ...
                <button id="simulate-vip-scan" onClick={simulateScan}
                    className="mt-3 w-full py-2.5 rounded-xl border border-slate-700 text-slate-400 hover:text-amber-400 hover:border-amber-500/40 text-xs font-bold transition-colors flex items-center justify-center gap-2">
            ...
            <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
                ...
                <input id="vip-scan-input" type="text" value={pastedId}
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-white font-mono placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-amber-500/40 focus:border-amber-500" />
                        ...
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                            ...
                            {result.ok ? (
                                <div className="space-y-1.5">
                                    ...
                                    <p><span className="text-slate-400">Guest:</span> {result.contact?.name}</p>
                                    <p><span className="text-slate-400">Tier:</span> <TierBadge tier={result.contact!.tier} /></p>
                                    <p><span className="text-slate-400">Agent Alerted:</span> {result.contact?.assignedAgentName}</p>
            ...
            {arrivals.length > 0 && (
                <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
                    ...
                    <div className="space-y-2">
                        {arrivals.map((a, i) => (
                            <div key={i} className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-3 py-2.5">
                                <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center flex-shrink-0">
                                    <Crown size={14} className="text-amber-400" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-white font-bold text-sm truncate">{a.leadName}</p>
                ...
                {agentAlert && (
                    <motion.div initial={{ x: 120, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 120, opacity: 0 }}
                        ...
                        <div className="bg-gradient-to-r from-amber-600 to-rose-600 p-4 flex items-start gap-3">
                            <Crown size={18} className="text-white flex-shrink-0 mt-0.5 animate-bounce" />
                            <div className="flex-1">
                                <p className="text-white font-extrabold text-sm">Agent Push Notification</p>
                                <p className="text-amber-100 text-xs mt-0.5 leading-relaxed">{agentAlert}</p>
                            </div>
                            <button onClick={() => setAgentAlert(null)} className="text-amber-200 hover:text-white flex-shrink-0">
                                <X size={16} />
                            </button>
                        </div>

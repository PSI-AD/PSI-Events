...
const TIER_CONFIG: Record<VIPTier, { label: string; color: string; bg: string; ring: string; icon: React.ReactNode }> = {
    ...
    Silver: { label: 'Silver', color: 'text-psi-secondary', bg: 'bg-psi-subtle0/15', ring: 'ring-slate-500/30', icon: <Star size={11} /> },
};
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-extrabold text-psi-primary text-sm truncate">{msg.clientName}</span>
                        <TierBadge tier={msg.clientTier} />
                    </div>
                    <div className="flex items-center gap-2 mb-2">
                        <Languages size={10} className="text-psi-muted flex-shrink-0" />
                        <span className="text-psi-muted text-[10px]">{msg.preferredLanguage}</span>
                        <span className="text-psi-muted">·</span>
                        <Clock size={10} className="text-psi-muted flex-shrink-0" />
                        <span className="text-psi-muted text-[10px]">{timeAgo(msg.receivedAt)}</span>
                    </div>
                    <p className="text-psi-secondary text-xs leading-relaxed line-clamp-2">{msg.body}</p>

                    {/* Status */}
                    <div className="flex items-center gap-1.5 mt-2">
                        <span className={`w-1.5 h-1.5 rounded-full ${STATUS_COLORS[msg.status]}`} />
                        <span className="text-[10px] font-bold text-psi-muted capitalize">{msg.status}</span>
                        {msg.assignedAgentName && (
                            <span className="text-[10px] text-psi-muted">→ {msg.assignedAgentName.split(' ')[0]}</span>
                        )}
                    </div>
                </div>
            </div>
...
function AgentPicker({
    ...
    const tierColors = { gold: 'text-amber-400', silver: 'text-psi-secondary', bronze: 'text-orange-400' };
    return (
                ...
                <motion.button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${selectedId === agent.id
                            ...
                            : agent.isPresent
                                ...
                                : 'bg-white/2 border-psi-subtle opacity-40 cursor-not-allowed'
                        }`}
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-psi-primary text-xs font-bold truncate">{agent.name}</p>
                        <p className="text-psi-primary/35 text-[10px]">{agent.branch} · {agent.languages.slice(0, 2).join(', ')}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className={`w-2 h-2 rounded-full ${agent.isPresent ? 'bg-emerald-400' : 'bg-psi-border'}`} />
...
export function AgentInterceptAlert({
    ...
    return (
        ...
        >
            ...
            <h2 className="text-psi-primary text-2xl font-extrabold mb-1">{task.clientName}</h2>
            <TierBadge tier={task.clientTier} />

            <div className="mt-6 bg-psi-subtle border border-psi rounded-2xl p-5 max-w-sm w-full text-left space-y-3">
                <div className="flex items-start gap-2">
                    <MessageCircle size={14} className="text-psi-muted flex-shrink-0 mt-0.5" />
                    <p className="text-psi-primary/70 text-sm italic leading-relaxed">"{task.snippet}"</p>
                </div>
                <div className="flex items-center gap-2">
                    <Languages size={13} className="text-psi-muted" />
                    <span className="text-psi-secondary text-xs">Speak in: <strong className="text-psi-primary">{task.preferredLanguage}</strong></span>
                </div>
                <div className="flex items-center gap-2">
                    <Phone size={13} className="text-psi-muted" />
                    <span className="text-psi-secondary text-xs">{task.clientPhone}</span>
                </div>
            </div>
            ...
            <p className="text-psi-muted text-xs mt-4">Go meet the client now. Task will be tracked.</p>
        </motion.div>
    );
}
                ...
                <div>
                    <div className="flex items-center gap-2 mb-0.5">
                        <div className="w-6 h-6 rounded-lg bg-emerald-500 flex items-center justify-center">
                            <MessageCircle size={13} className="text-psi-primary" />
                        </div>
                    ...
                    <h1 className="text-psi-primary text-xl font-extrabold">Concierge Control Center</h1>
                </div>
                        ...
                        {(['all', 'new', 'dispatched', 'completed'] as const).map(s => (
                            <button key={s} id={`vip-filter-${s}`}
                                ...
                                className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${filterStatus === s
                                        ...
                                        : 'bg-psi-subtle text-psi-muted hover:text-psi-primary/70'
                                    }`}
                            >
                        ...
                        {selectedMsg ? (
                            <motion.div key={selectedMsg.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                ...
                                <div className="px-6 py-4 border-b border-white/8 flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            <h2 className="text-psi-primary font-extrabold">{selectedMsg.clientName}</h2>
                                            <TierBadge tier={selectedMsg.clientTier} />
                                        </div>
                                        <div className="flex items-center gap-3 mt-0.5">
                                            <span className="text-psi-muted text-xs flex items-center gap-1"><Phone size={10} />{selectedMsg.clientPhone}</span>
                                            <span className="text-psi-muted text-xs flex items-center gap-1"><Languages size={10} />{selectedMsg.preferredLanguage}</span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex-1 overflow-y-auto px-6 py-6">
                                    <div className="max-w-lg">
                                        ...
                                        <div className="bg-[#25D366]/10 border border-[#25D366]/25 rounded-2xl rounded-tl-sm px-4 py-3 mb-2">
                                            <p className="text-psi-primary/90 text-sm leading-relaxed">{selectedMsg.body}</p>
                                        </div>
                                        <div className="flex items-center gap-2 px-1">
                                            <div className="w-4 h-4 rounded-full bg-[#25D366] flex items-center justify-center flex-shrink-0">
                                                <MessageCircle size={9} className="text-psi-primary" />
                                            </div>
                                            <span className="text-psi-muted text-[10px]">via WhatsApp · {timeAgo(selectedMsg.receivedAt)}</span>
                                        </div>
                                    </div>
                                ...
                                {selectedMsg.status === 'new' && (
                                    <div className="border-t border-white/8 px-6 py-5 space-y-4 bg-psi-surface/60">
                                        <div className="flex items-center gap-2">
                                            <Zap size={14} className="text-indigo-400" />
                                            <p className="text-psi-primary font-extrabold text-sm">Assign Intercept Task</p>
                                        </div>
                            ...
                            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                className="flex-1 flex flex-col items-center justify-center text-psi-muted gap-3">
                ...
                <div className="hidden lg:flex w-72 flex-col border-l border-white/8 overflow-hidden">
                    <div className="px-5 py-4 border-b border-white/8">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-psi-primary font-extrabold text-sm">Live Task Board</p>
                        </div>
                        <p className="text-psi-muted text-[10px] mt-0.5">{tasks.length || agents.filter(a => a.activeTasks > 0).length} active assignments</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                            ...
                            {(tasks.length > 0 ? tasks : messages.filter(m => m.status === 'dispatched' || m.status === 'completed').map(m => ({
                                ...
                                <motion.div key={task.id} layout initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }}
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-psi-primary text-xs font-bold truncate">{task.clientName}</span>
                                    ...
                                    <div className="flex items-center gap-1.5 text-[10px] text-psi-muted">
                                        <ArrowRight size={9} />
                        ...
                        {tasks.length === 0 && messages.filter(m => m.status !== 'new').length === 0 && (
                            <div className="text-center py-10 text-psi-primary/15">

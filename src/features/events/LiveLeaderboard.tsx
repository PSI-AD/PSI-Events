...
export default function LiveLeaderboard({
    ...
    return (
        <div className="min-h-screen bg-psi-page select-none overflow-x-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                ...
                {[
                    ...
                    <div key={label} className={`${bg} rounded-2xl px-3 py-3 text-center`}>
                        ...
                        <p className="text-[10px] text-psi-muted font-bold uppercase tracking-wider mt-0.5">{label}</p>
                    </div>
                ))}
            </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-psi-primary font-extrabold text-xs">
                                {getInitials(myEntry.agentName)}
                            </div>
                            <div>
                                <p className="text-psi-primary font-bold text-sm">{myEntry.agentName}</p>
                                <p className="text-blue-400 text-[10px] font-bold">#{myRank} of {entries.length} on floor</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-2xl font-extrabold text-psi-primary font-mono leading-none">
                                <AnimatedNumber value={myEntry.leadsToday} />
                            </p>
                            <p className="text-[10px] text-psi-muted font-bold">leads today</p>
                        </div>
                    </div>
                ...
                {viewMode === 'individual' ? (
                    <motion.div key="individual" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-[10px] font-bold text-psi-muted uppercase tracking-[0.15em] flex items-center gap-1.5">

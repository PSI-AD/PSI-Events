...
export function MediaPreviewStrip({
    ...
    return (
        <div className="px-4 py-3 border-t border-psi-subtle">
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                {files.map((f, i) => {
                    const isImg = f.type.startsWith('image/');
                    const url = isImg ? URL.createObjectURL(f) : null;
                    return (
                        <div key={i} className="relative flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-psi-subtle border border-psi">
                            {url ? (
                                <img src={url} alt="" className="w-full h-full object-cover" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center">
                                    <Video size={20} className="text-psi-secondary" />
                                </div>
                            )}
                            <button
                                onClick={() => onRemove(i)}
                            ...
                            >
                                <X size={10} className="text-psi-primary" />
                            </button>
                        </div>
                    );
                })}
            ...
            {uploadProgress !== null && (
                <div className="mt-2">
                    <div className="h-1 bg-psi-border-subtle rounded-full overflow-hidden">
                        <motion.div
                    ...
                    <p className="text-[10px] text-psi-muted mt-1">{uploadProgress}% uploaded</p>
                </div>
            )}
        </div>
    );
            ...
            <AnimatePresence>
                {!open && (
                    <motion.button
                        ...
                        className="fixed bottom-6 right-6 z-40 w-14 h-14 bg-gradient-to-br from-violet-500 to-violet-700 rounded-2xl shadow-2xl shadow-violet-500/30 flex items-center justify-center text-psi-primary hover:scale-105 active:scale-95 transition-transform"
                ...
                {open && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed bottom-0 left-0 right-0 z-50 bg-psi-surface border-t border-psi rounded-t-3xl pb-safe"
                            onClick={e => e.stopPropagation()}
                        >
                            <div className="flex justify-center pt-3 pb-1">
                                <div className="w-10 h-1 bg-psi-border rounded-full" />
                            </div>
                            <div className="px-4 py-3 flex items-center justify-between">
                                <div>
                                    <p className="font-extrabold text-psi-primary text-sm">New Journal Post</p>
                                    <p className="text-[11px] text-psi-muted">{eventName}</p>
                                </div>
                                <button onClick={() => setOpen(false)}
                                    className="w-8 h-8 rounded-full bg-psi-subtle flex items-center justify-center text-psi-secondary hover:bg-psi-subtle transition-colors">
                                    <X size={16} />
                                </button>
                            </div>
                            ...
                            <div className="px-4 pb-2">
                                <textarea
                                    ...
                                    rows={5}
                                    className="w-full bg-psi-subtle/60 border border-white/8 rounded-2xl px-4 py-3 text-sm text-psi-primary placeholder:text-psi-muted resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/40 transition-all leading-relaxed"
                                />
                            </div>
                            ...
                            <div className="px-4 py-3 flex items-center gap-2">
                                <button id="journal-upload-btn" onClick={() => fileInputRef.current?.click()}
                                    className="w-12 h-12 rounded-2xl bg-psi-subtle border border-white/8 flex items-center justify-center text-psi-secondary hover:text-psi-primary hover:bg-slate-700 transition-all active:scale-95"
                                ...
                                <button id="journal-dictate-btn"
                                    ...
                                    className={`w-12 h-12 rounded-2xl border flex items-center justify-center transition-all active:scale-95
                                        ${dictState === 'listening'
                                            ? 'bg-rose-500 border-rose-500 text-psi-primary shadow-lg shadow-rose-500/30 animate-pulse'
                                            : dictState === 'error'
                                                ? 'bg-psi-subtle border-rose-500/30 text-rose-400'
                                                : 'bg-psi-subtle border-white/8 text-psi-secondary hover:text-psi-primary hover:bg-slate-700'
                                        }
                                    ...
                                    className={`flex-1 h-12 rounded-2xl border flex items-center justify-center gap-2 text-sm font-bold transition-all active:scale-[0.97]
                                        ${polishing
                                            ...
                                            : text.trim()
                                                ...
                                                : 'bg-psi-subtle border-white/8 text-psi-muted cursor-not-allowed'
                                        }
                                    `}
                                ...
                                <button id="journal-post-btn" onClick={handlePost} disabled={!canPost}
                                    className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all active:scale-95
                                        ${canPost
                                            ? 'bg-emerald-500 text-psi-primary shadow-lg shadow-emerald-500/20 hover:bg-emerald-400'
            ...
            <AnimatePresence>
                {successMsg && (
                    <motion.div
                        ...
                        className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-3 bg-emerald-500 text-psi-primary rounded-2xl shadow-2xl font-bold text-sm">

        ...
        <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3">
                <div className={clsx(
                    ...
                    rank === 1 ? 'bg-amber-400 text-amber-900' :
                        rank === 2 ? 'bg-slate-300 text-slate-900' :
                            rank === 3 ? 'bg-orange-600 text-psi-primary' : 'bg-psi-border text-psi-secondary'
                )}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        <p className="text-psi-primary font-extrabold text-sm truncate">{stat.branchName}</p>
                        <span className="text-[9px] text-psi-muted font-bold">{stat.agentCount} agents</span>
                    </div>
                ...
                <div className="text-right flex-shrink-0">
                    <p className={clsx('text-2xl font-black font-mono leading-none', rank === 1 ? 'text-amber-300' : 'text-psi-primary')}>
                        <AnimatedNumber value={stat.totalLeads} />
                    </p>
                    <p className="text-[10px] text-psi-muted mt-0.5">leads</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
                {stat.agents.sort((a, b) => b.leadsToday - a.leadsToday).map(a => (
                    <span key={a.agentId} className="inline-flex items-center gap-1 bg-psi-subtle border border-white/8 rounded-full px-2 py-0.5 text-[10px] text-psi-secondary font-bold">
                        <span className={`w-4 h-4 rounded-full bg-gradient-to-br ${getTierGradient(a.tier)} inline-flex items-center justify-center text-[8px] font-black text-psi-primary`}>
    ...
    return (
        <div className="mx-4 mb-2">
            ...
            <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex-1 text-left">
                    <div className={clsx(
                        ...
                        isLeftLeading ? 'bg-amber-500/20 border border-amber-400/50 text-amber-300 shadow-lg shadow-amber-500/20' : 'bg-psi-subtle border border-psi text-psi-secondary'
                    )}>
                        <Building2 size={11} />
                        {left.branchName}
                        {isLeftLeading && !isTie && <Crown size={10} className="text-amber-400 animate-pulse" />}
                    </div>
                    <p className={clsx('text-3xl font-black font-mono mt-2 ml-1 transition-all', isLeftLeading ? 'text-amber-300' : 'text-psi-secondary')}>
                        <AnimatedNumber value={left.totalLeads} />
                    </p>
                    <p className="text-[10px] text-psi-muted font-bold ml-1">{left.agentCount} agent{left.agentCount !== 1 ? 's' : ''}</p>
                </div>

                {/* VS badge */}
                <div className="flex flex-col items-center justify-center px-3 pt-1">
                    <div className={clsx('w-10 h-10 rounded-full border-2 flex items-center justify-center', isTie ? 'border-slate-500 bg-psi-subtle' : 'border-amber-500/50 bg-amber-500/10')}>
                        <Swords size={14} className={isTie ? 'text-psi-secondary' : 'text-amber-400'} />
                    </div>
                    <p className="text-[9px] text-psi-muted font-black uppercase tracking-widest mt-1">VS</p>
                </div>
                ...
                <div className="flex-1 text-right">
                    <div className={clsx(
                        'inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition-all',
                        !isLeftLeading && !isTie ? 'bg-amber-500/20 border border-amber-400/50 text-amber-300 shadow-lg shadow-amber-500/20' : 'bg-psi-subtle border border-psi text-psi-secondary'
                    )}>
                        {!isLeftLeading && !isTie && <Crown size={10} className="text-amber-400 animate-pulse" />}
                        {right.branchName}
                        <Building2 size={11} />
                    </div>
                    <p className={clsx('text-3xl font-black font-mono mt-2 mr-1 transition-all', !isLeftLeading && !isTie ? 'text-amber-300' : 'text-psi-secondary')}>
                        <AnimatedNumber value={right.totalLeads} />
                    </p>
                    <p className="text-[10px] text-psi-muted font-bold mr-1">{right.agentCount} agent{right.agentCount !== 1 ? 's' : ''}</p>
                </div>
            </div>

            {/* The Tug-of-War bar */}
            <div className="relative h-8 rounded-full overflow-hidden bg-psi-subtle shadow-inner shadow-black/50">
                <motion.div
                ...
                <div className="absolute inset-y-0 left-1/2 -translate-x-1/2 flex items-center z-10">
                    <div className={clsx(
                        ...
                        isTie ? 'bg-psi-border border-slate-500 text-psi-secondary' : 'bg-amber-500 border-amber-300 text-amber-900 shadow-amber-500/50'
                    )}>
                        {isTie ? '=' : isLeftLeading ? '◀' : '▶'}
                    </div>
                </div>
            ...
            <motion.div key={gap} initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                ...
                {isTie ? (
                    <p className="text-psi-secondary text-xs font-bold">⚖️ Tied — it's anyone's game!</p>
                ) : (
                    <p className="text-xs font-extrabold">
                        <span className="text-amber-400">{isLeftLeading ? left.branchName : right.branchName}</span>
                        <span className="text-psi-secondary mx-1.5">leads by</span>
        ...
        <div className="space-y-4 pb-20">
            ...
            {top && challenger ? (
                <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                    className="mx-4 bg-psi-surface border border-white/8 rounded-3xl p-5 relative overflow-hidden">
                ...
                <div className="mx-4 text-center py-6 text-psi-muted text-sm">
            ...
            {branches.length > 0 && (
                <div className="px-4 space-y-2">
                    <p className="text-[10px] font-bold text-psi-muted uppercase tracking-[0.15em] flex items-center gap-1.5 mb-3">

        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="ml-auto flex items-center gap-2.5">
                    ...
                    <a
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold transition-all shadow-lg shadow-emerald-600/20"
                    >
            ...
            <motion.div
                ...
                className={clsx(
                    ...
                    'border-white/10 text-white shadow-2xl',
                )}
            >
                {/* Hero image */}
                {project.imageUrl && (
                    <div className="relative h-52 overflow-hidden">
                        ...
                        {/* Language badge */}
                        <div className="absolute top-4 left-4 flex items-center gap-2 bg-black/40 backdrop-blur-sm border border-white/20 rounded-xl px-3 py-1.5">
                            <span className="text-lg">{content.language.flag}</span>
                            <span className="text-white text-xs font-bold">{content.language.nativeLabel}</span>
                            {isRTL && (
                                <span className="text-white/50 text-[9px] font-bold uppercase tracking-widest border border-white/20 rounded px-1">RTL</span>
                            )}
                        </div>
                        ...
                        <div className="absolute bottom-4 left-5 right-5">
                            ...
                            <h2 className="text-white text-2xl font-extrabold leading-tight">{content.projectName}</h2>
                        </div>
                    </div>
                )}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[
                            ...
                            <div key={label} className="bg-white/5 border border-white/10 rounded-2xl p-3 text-center">
                                ...
                                <p className="text-[9px] text-white/40 font-bold uppercase tracking-widest mb-0.5">{label}</p>
                                <p className="text-white text-xs font-extrabold leading-tight">{value}</p>
                            </div>
                        ))}
                    </div>

                    {/* Description */}
                    <div className={clsx('text-white/70 text-sm leading-relaxed', isRTL && 'text-right')}>
                        {content.description}
                    </div>
                    ...
                    <div className="space-y-2">
                        {content.highlights.map((h, i) => (
                            ...
                            >
                                ...
                                <span className="text-white/80 text-sm">{h.replace(/^[◆✦•◈▸] /, '')}</span>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <div className={clsx('flex items-start gap-4', isRTL && 'flex-row-reverse')}>
                        <img
                            ...
                            alt={DEMO_AGENT.name}
                            className="w-14 h-14 rounded-2xl object-cover border border-white/20 flex-shrink-0 bg-slate-700"
                        />
                        <div className={clsx('flex-1 min-w-0', isRTL && 'text-right')}>
                            <p className="text-white font-extrabold text-sm">{DEMO_AGENT.name}</p>
                            <p className="text-amber-400 text-[10px] font-bold uppercase tracking-widest mb-2">{DEMO_AGENT.branch}</p>
                            <p className="text-white/60 text-xs leading-relaxed">{content.agentBio}</p>
                        </div>
                    </div>

                    {/* Closing / CTA */}
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 text-center space-y-3">
                        <p className={clsx('text-white/70 text-xs leading-relaxed', isRTL && 'text-right')}>
                            {content.closingNote}
                        </p>
                        <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-white font-extrabold text-sm shadow-lg shadow-amber-500/25">
                            <Send size={13} />
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <motion.button
                ...
                type="submit"
                whileTap={{ scale: 0.97 }}
                className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl bg-gradient-to-r from-amber-500 via-amber-400 to-orange-400 text-white font-extrabold text-base shadow-xl shadow-amber-500/25 hover:shadow-amber-500/40 transition-all"
            >
                <Sparkles size={18} />
                            ...
                            <div className={clsx(
                                ...
                                <div className={clsx(
                                    ...
                                    step === s ? 'bg-amber-500 border-amber-500 text-white' :
                                        i < ['form', 'generating', 'preview'].indexOf(step) ? 'bg-emerald-500 border-emerald-500 text-white' :

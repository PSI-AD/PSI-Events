...
const CATEGORIES: { value: IntelCategory; label: string; icon: React.ReactNode; color: string }[] = [
    ...
    { value: 'other', label: 'Other', icon: <Eye size={13} />, color: 'text-slate-400 bg-slate-500/10 border-slate-500/25' },
];
    ...
    return (
        <>
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={handleOpen}
                className="fixed bottom-24 right-5 z-40 w-14 h-14 rounded-2xl bg-gradient-to-br from-rose-500 to-rose-600 text-white shadow-2xl shadow-rose-600/40 flex items-center justify-center hover:from-rose-400 hover:to-rose-500 transition-all"
                    ...
                    >
                        ...
                        {phase === 'idle' && (
                            <motion.div
                                ...
                                className="mt-auto bg-slate-900 rounded-t-3xl p-6 space-y-6 border-t border-white/10"
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-[10px] font-black tracking-[0.2em] uppercase text-rose-400 mb-0.5">Market Intelligence</p>
                                        <h2 className="text-white text-xl font-extrabold">Drop Competitor Intel</h2>
                                    </div>
                                    <button id="intel-fab-close" onClick={handleClose} className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center text-white/60 hover:text-white">
                                        <X size={18} />
                                    </button>
                                </div>
                                ...
                                {/* Category */}
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-3">Category</p>
                                ...
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-2">Quick Caption (Optional)</p>
                                    <input
                                        type="text"
                                        value={voiceNote}
                                        maxLength={120}
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20"
                                    />
                                </div>
                        ...
                        {phase === 'capture' && (
                            <div className="flex-1 flex flex-col">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                    <div className="w-72 h-52 relative">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <span className="text-white/50 text-xs font-bold tracking-widest uppercase">Aim at competitor stand</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-8 flex items-center justify-center gap-8 bg-gradient-to-t from-black/60 to-transparent">
                                    <button onClick={handleClose} className="w-12 h-12 rounded-full bg-white/15 flex items-center justify-center text-white">
                                        <X size={22} />
                                    </button>
                                    <motion.button
                                        id="intel-snap-btn"
                                    ...
                                    <button onClick={() => fileInputRef.current?.click()} className="w-12 h-12 rounded-full bg-white/15 flex items-center justify-center text-white">
                                        <Image size={22} />
                                    </button>
                                </div>
                            </div>
                        ...
                        {phase === 'preview' && previewUrl && (
                            ...
                            >
                                ...
                                {/* Bottom panel */}
                                <div className="bg-slate-900 p-5 space-y-4 border-t border-white/10">
                                    <div className="flex items-center justify-between">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/25 focus:outline-none focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/20"
                                    />
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button onClick={handleClose}
                                            className="py-3.5 rounded-2xl bg-white/5 border border-white/10 text-white/60 text-sm font-bold hover:bg-white/10 transition-all">
                                            Cancel
                                        </button>
                                        <motion.button
                                            id="intel-submit-btn"
                        ...
                        {phase === 'analysing' && (
                            <div className="flex-1 flex flex-col items-center justify-center gap-6 text-white">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-white/40 text-sm">Extracting competitor intel from your image</p>
                                </div>
                            </div>
                        )}
                        ...
                        {phase === 'done' && (
                            ...
                            >
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-white/50 text-sm">Your intel is live on the HQ dashboard. Organizers can see it in real-time.</p>
                                </div>
                                <button id="intel-done-btn" onClick={handleClose}
                                    className="px-8 py-3.5 rounded-2xl bg-white/10 border border-white/15 text-white font-bold hover:bg-white/15 transition-all">
                                    Back to Floor
                                </button>
                            </motion.div>
                        )}
...
function IntelCard({ drop, onClick }: { drop: IntelDrop; onClick: () => void }) {
    ...
    return (
        ...
        >
            {/* Image */}
            <div className="relative overflow-hidden bg-slate-800" style={{ height: `${Math.floor(180 + Math.random() * 120)}px` }}>
                <img
                    src={drop.imageUrl}
                ...
                {/* Expand icon */}
                <button className="absolute top-3 right-3 w-7 h-7 rounded-lg bg-black/40 backdrop-blur-sm border border-white/15 flex items-center justify-center text-white/70 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Maximize2 size={12} />
                </button>

                {/* Competitor name overlay */}
                <div className="absolute bottom-3 left-3 right-3">
                    <p className="text-white text-xs font-black truncate">{drop.competitorName}</p>
                </div>
            </div>
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-1.5">
                        <div className="w-5 h-5 rounded-full bg-gradient-to-br from-rose-500 to-rose-600 flex items-center justify-center text-white font-black text-[9px]">
...
function IntelLightbox({ drop, onClose }: { drop: IntelDrop; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            {/* Side panel */}
            <div className="w-full md:w-96 bg-slate-900 border-t md:border-t-0 md:border-l border-white/10 flex flex-col overflow-y-auto">
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-white/10">
                    ...
                    <button id="intel-lb-close" onClick={onClose}
                        className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center text-white/60 hover:text-white">
                        <X size={18} />
                    </button>
                </div>
                ...
                <div className="p-5 space-y-5 flex-1">
                    {/* Competitor */}
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/30 mb-1">Competitor</p>
                        <p className="text-white text-xl font-extrabold">{drop.competitorName}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="bg-rose-500/8 border border-rose-500/15 rounded-2xl p-4">
                            <p className="text-white text-sm leading-relaxed">{drop.aiSummary}</p>
                        </div>
                    </div>
                    ...
                    {drop.voiceNote && (
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest text-white/30">Agent Note</p>
                            </div>
                            <p className="text-white/60 text-sm italic">&ldquo;{drop.voiceNote}&rdquo;</p>
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-white/10">
                        <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-rose-500 to-rose-600 flex items-center justify-center text-white font-black text-xs">
                                {drop.agentName.charAt(0)}
                            </div>
                            <div>
                                <p className="text-white text-xs font-bold">{drop.agentName}</p>
                                <p className="text-white/30 text-[10px]">Field agent</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-white/30 text-[10px]">{timeAgo(drop.createdAt)}</p>
                        </div>
                    </div>
                </div>
            </div>
